...
def one(i):
    with sem:
        ...
        prompt = f"req{i} unique seed {i*7919}. " + "the archival index records that fact again " * IN_REPEAT
        body = json.dumps({"model": MODEL, "prompt": prompt, "max_tokens": OUT, "temperature": 0}).encode()
        t0 = time.time()
        try:
            d = json.loads(urllib.request.urlopen(
                urllib.request.Request(BASE, data=body, headers={"Content-Type": "application/json"}),
                timeout=600).read())
            dt = time.time() - t0
...
print(f"[{TAG}] decode_tok/s={tot_ct/wall:.1f} total_tok/s={(tot_ct+tot_pt)/wall:.1f} "
