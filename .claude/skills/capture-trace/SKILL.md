...
| `--profiler-max-iters` | Stop recording after N of *that engine's* steps regardless of how long the bench runs. Server flag, or `max_iters` in the `/start_profile` body when the server is already up | `200`, or omit and rely on a short bench |
