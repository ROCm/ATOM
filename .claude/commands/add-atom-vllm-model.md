...
- [ ] 8) `recpies/vllm_atom` recipe added/updated
...
`recpies/vllm_atom/<Model-Name>.md` section using this table format:
...
Use existing files in `recpies/vllm_atom/` (such as `DeepSeek-R1.md`, `Qwen3.5.md`, `Kimi-K2.5.md`) as style references.
...
Update `.github/workflows/atom-vllm-vLLM-ATOM-test.yaml` and add the model entry to the
**nightly accuracy path only**.

Do **not** add this model to the pull-request vLLM-ATOM matrix (`jobs.atom-vllm-vLLM-ATOM.strategy.matrix.include`).
