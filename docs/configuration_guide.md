...
| `profiler_delay_iters` | `int` | `0` | Engine steps to skip after `/start_profile` before the profiler records. `0` records immediately. Counted per `/start_profile`, so the delay restarts on every request. Each engine counts its own steps. A request may carry its own `delay_iters`, which applies to that run only and leaves this default alone |
| `profiler_max_iters` | `int` | `0` | Recorded engine steps after which the profiler stops itself and writes the trace, with no `/stop_profile` needed. `0` records until stopped. An engine step is one forward dispatch, which under MTP or EAGLE covers the draft passes too. Each engine counts its own steps, so the same number is not the same work or wall time everywhere. On a pipeline-parallel head a step is one microbatch, and the window closes at the end of the pass that fills it, so the trace holds whole pipeline waves. Across separate prefill and decode servers, send each its own `/start_profile` with the window that suits it. Overridable per request by `max_iters`, so a window can be changed without restarting the server |
...
| `--profiler-delay-iters` | | `int` | `0` | Engine steps to skip after `/start_profile` before recording starts. Default for the request body's `delay_iters` |
| `--profiler-max-iters` | | `int` | `0` | Stop the profiler and write the trace after this many recorded engine steps, without a `/stop_profile` call. Default for the request body's `max_iters` |
