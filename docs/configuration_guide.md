...
| `profiler_delay_iters` | `int` | `0` | Engine steps to skip after `/start_profile` before the profiler records. `0` records immediately. Counted per `/start_profile`, so the skid restarts on every request |
