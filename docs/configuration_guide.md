...
| `state_checkpoint_slots` | `int` | `0` | Spare Active Slots kept for checkpoints to sit in, on top of the per-request floor. Only a `fork` backend needs them; under PAGE the image lives in KV blocks, so the backend zeroes this out rather than reserve slots that would hold nothing |
...
| `--state-checkpoint-slots` | `--state-checkpoint-groups` | `int` | `0` | Spare Active Slots for checkpoints to occupy, on top of the per-request floor. Only a `fork` backend uses them; a PAGE backend keeps its images in KV blocks and zeroes this out. The old spelling is still accepted |
