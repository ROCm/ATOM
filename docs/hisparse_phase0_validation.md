...
HiSparse Phase 0 **功能完整、精度无损、换入(swap)正确**,端到端跑通 GLM-5.2 PD decode。
...
三项叠加证明: HiSparse 在**有/无 swap 两种情况下都数值正确**。

## 2. 测试详情

### 2.1 Swap kernel 单元测试(GPU 直测)

独立验证 HIP gather kernel(`atom/hisparse/swap_kernel.py`)从 pinned host cold pool
...
> (32 会在思考中途被截断,是探针问题,非 HiSparse 问题)。

## 3. 脚本

| 脚本 | 作用 |
|------|------|
| `scripts/start_glm52_pp4pd_hisparse.sh` | 启动 PP4 prefill + TP4 decode,decode 端开 HiSparse |
| `scripts/test_hisparse_needle.py` | 长上下文换入压测(needle 召回) |
| `tests/test_hisparse_coordinator.py` | coordinator 纯逻辑单测(14 项,CPU,无需 GPU) |

### 3.1 启动(decode 端 HiSparse)
...
- `ATOM_HISPARSE_ENABLE=1`(仅 decode 端)。
...
docker exec -it \
  ...
  atom_pp4pd_test bash scripts/start_glm52_pp4pd_hisparse.sh
```
...
```bash
# python scripts/test_hisparse_needle.py [PORT] [FILLER_LINES] [DEPTH_FRAC]
docker exec -w /it-share/yajizhan/code/ATOM atom_pp4pd_test \
  python scripts/test_hisparse_needle.py 30000 650 0.08
...
- **性能是 Phase 1**:当前每层 per-token Python miss-detect + 每层 D2H 同步,
  ...
  把 miss-detect/LRU fuse 进 HIP kernel(对标 SGLang `hisparse.cuh`)、CUDAGraph 兼容。
- 更大规模精度(全量 1319)与更长上下文(62K,对标设计文档附录 A)可进一步压测。

## 5. 实现落点(参见 `.claude/plans/hisparse_phase0_plan.md`)

```
atom/hisparse/{__init__,swap_kernel,coordinator}.py   # 新增: kernel + coordinator
atom/utils/envs.py                                     # ATOM_HISPARSE_ENABLE / _HOT_BUFFER_SIZE
