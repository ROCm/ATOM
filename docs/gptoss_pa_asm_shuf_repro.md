...
```text
ATOM_GPTOSS_USE_PA_DECODE_BF16_ASM enabled: routing matching GPT-OSS decode layers through aiter.pa_decode_bf16_asm
...
```text
ATOM_GPTOSS_USE_PA_DECODE_BF16_ASM fallback: sliding-window layer
ATOM_GPTOSS_USE_PA_DECODE_BF16_ASM fallback: prefill
```
