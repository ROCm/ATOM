...
| **ATOM_TTFT_TRACE** | bool | 0 (false) | Set to `1` to emit `record_function` spans at the decode TTFT stage boundaries: `ttft[api_preprocess]`, `ttft[api_enqueue]`, `ttft[prepare_model]`, `ttft[gpu_forward]`, `ttft[postprocess]`. Needed to split `atom:ttft_forward_to_output_seconds` inside a trace; the histograms alone cannot. Independent of `ATOM_TORCH_PROFILER_DIR`, but only visible in a captured trace. When off, each site still builds a no-op context manager per forward. See `docs/ttft_breakdown_guide.md`. |
