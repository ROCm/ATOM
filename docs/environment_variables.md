...
| **ATOM_V4_TRITON_NATIVE_BF16_V** | bool | 0 (false) | On gfx950, enable the experimental native FP8-to-BF16 V path for eligible HCA V4 decode shapes. DP CSA B10+ selects its validated native-V schedule automatically. With the default hybrid policy, this also enables the tuned B6 HCA Triton specialization. |
| **ATOM_V4_TRITON_HYBRID_DECODE** | bool | 1 (true) | On gfx950 with `ATOM_USE_TRITON_ATTN=1`, keep Triton for measured native-FP8 winners while routing slower DP-attention HCA shapes to AITER. DP CSA remains Triton, including its native-V B10+ schedule. The optimized B6 HCA path is enabled by `ATOM_V4_TRITON_NATIVE_BF16_V=1`. Set `0` to force Triton for every supported native-FP8 shape. Other GPU architectures remain all-Triton. |
| **ATOM_V4_TRITON_FP8_PREFILL** | bool | 0 (false) | On gfx950, opt into the experimental native two-buffer FP8 sparse-prefill Triton candidate. Requires the Triton attention master path; unset keeps the validated AITER OPUS prefill kernel while tuning and long-context qualification are in progress. |
| **ATOM_V4_FLYDSL_FP8_DECODE** | bool | 0 (false) | On gfx950, opt into the graph-safe tuned FlyDSL split-K kernel for native-FP8 H=128/q7 HCA B6 decode. Other decode shapes retain the Triton/AITER hybrid policy. Requires `ATOM_USE_TRITON_ATTN=1`. |
| **ATOM_V4_FLYDSL_FP8_PREFILL** | bool | 0 (false) | On gfx950, opt into the tuned H=128 FlyDSL native-FP8 prefill kernel for sentinel-free inputs with scheduler-owned `max_seqlen_q <= 127` and `max_seqlen_k < 4096`. All other inputs remain on AITER OPUS. Requires `ATOM_USE_TRITON_ATTN=1`; `ATOM_FORCE_V4_PREFILL_OPUS=1` overrides it. |
