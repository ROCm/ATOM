...
| **ATOM_USE_FLYDSL_GATHER_KV_B_PROJ** | bool | 1 (true) | Use the FlyDSL fused gather + `kv_b_proj` GEMM for MLA's cached-prefix path. Covers page_size-1 fp8 (e4m3) KV with an fp8 weight on gfx950 — i.e. Kimi-K3 / DeepSeek MLA under `--kv-cache-dtype fp8`. Unavailable imports use Triton; kernel validation and execution errors propagate. Set `0` to force Triton. |
| **ATOM_USE_FLYDSL_FP8_PREFILL_ATTN** | bool | 0 (false) | Use FlyDSL FP8 FMHA for MLA prefill on gfx950. Quantizes Q/K/V to E4M3 and folds the layer softmax scale into Q's descale. Requires BF16 output, Q/K head dimension divisible by 64, and V dimension 64–192 divisible by 32. Unavailable kernels or unsupported calls use AITER varlen attention. Added 2026-09-10. |
