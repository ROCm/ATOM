...
| Variable | Type | Default | Description |
|----------|------|---------|-------------|
| **ATOM_USE_TRITON_ATTN** | bool | 1 (true) | Master switch for custom attention kernels. For native two-buffer FP8 V4 decode, `1` permits the qualified FlyDSL path and `0` forces the available AITER/ASM fallback. The name is retained for compatibility with legacy Triton attention paths. |
| **ATOM_V4_FLYDSL_FP8_DECODE** | bool | 1 (true) | On gfx950, use graph-safe FlyDSL native-FP8 decode for qualified H128/q7 CSA B1-B32 and HCA B6 shapes. Set `0` to force AITER/ASM for native-FP8 decode. `ATOM_USE_TRITON_ATTN=0` also forces the fallback. |
