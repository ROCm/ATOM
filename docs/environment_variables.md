...
| Variable | Type | Default | Description |
|----------|------|---------|-------------|
| **ATOM_GC_FREEZE** | bool | 1 (true) | Move the startup heap into CPython's permanent generation once warmup is done, so collections stop scanning it. Applied in the API server, every EngineCore and every ModelRunner worker; undone on engine shutdown so an in-process teardown does not leak. Disaggregated decode is frozen before its block pool exists (it signals ready first), so it covers the model, graph and tokenizer but not the pool. Set `0` to keep the pre-freeze behaviour. |
