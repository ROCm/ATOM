...
| **ATOM_MORI_FP8_DISPATCH** | bool | 0 (false) | If set to `1`, quantize activations to MXFP8 (fp8 e4m3, one e8m0 scale per 32 elements) before the MoE all2all instead of sending bf16 — about half the bytes on the dispatch wire (7168 + 224 B per token at hidden 7168, vs 14336) — which selects `EpDispatchIntraNodeKernel_fp8_ocp`. This is the quantization aiter's a8w4 `fused_moe` (MXFP4 weights, fp8 activations) otherwise runs on the received rows, so aiter takes the pre-quantized rows as-is and only sorts the scales. Requires MXFP4 routed experts and an aiter config that always picks fp8 activations (on gfx950: `ATOM_MOE_GU_ITLV=1` and `AITER_BF16_FP8_MOE_BOUND=0`); the first MoE forward raises otherwise. Mutually exclusive with `ATOM_MORI_FP4_DISPATCH`. Resolved together with the staging scale geometry by `mori_prepare_finalize.resolve_mori_dispatch()`. |
...
| **ATOM_MORI_LAUNCH_POLICY** | str | `tuned` | Launch geometry (block count, warps per block) of the MoRI IntraNode dispatch and combine kernels. `tuned` picks each phase separately from MoRI's shipped tuning tables (`mori/ops/tuning_configs/{arch}_{model}_IntraNode_ep{N}_{dispatch,combine}.json`), keyed by the largest per-rank token count in the DP group, with block count capped at the CU count and warps at 16; it falls back to `legacy` when no table matches. `legacy` keeps the fixed grid keyed on this rank's own prefill flag: 128 blocks x 16 warps on a prefilling rank, 64 blocks x 4 warps otherwise, for both phases; non-IntraNode kernels always use it. The chosen tables are logged once on rank 0. |
| **ATOM_MORI_MASK_PAD_ROWS** | bool | 0 (false) | If set to `1`, the MoRI IntraNode prepare routes this rank's DP pad rows (rows past `scheduled_tokens` on a step padded to the group's `running_tokens`, target and draft passes alike) to expert `-1`. The dispatch kernel sends nothing for a negative id and combine returns zeros for a token sent nowhere, so padding stops costing transport and expert GEMM, and identical pad rows stop piling onto the same experts. The rows are selected by a device mask (`row >= scheduled_tokens`) that `set_forward_context` and the drafters publish before each pass, so a CUDA graph replay follows its own step and each MoE layer pays one extra select kernel. Ignored for TBO ubatches, non-IntraNode kernels and plugin frontends. |
| **ATOM_MORI_ZERO_COPY_COMBINE** | str | `0` | Transport of the synchronous MoRI IntraNode combine. `0` keeps the push combine: each rank copies every received row into the sender's staging slot over XGMI, then sums its local staging. `1` and `copy` switch to MoRI's zero-copy (pull) combine on the same handle and buffers: the expert output sits in the rank's registered combine input buffer and each rank reads its tokens' rows straight from its peers'. With `1`, AITER `fused_moe` writes its output directly into that buffer (`output=`), including the zero-fill and stage-2 accumulation; with `copy`, `fused_moe` keeps its own buffer and the received rows are copied in locally before the combine. The launch geometry follows MoRI's zero-copy combine tuning rules. Needs `ATOM_MORI_COMBINE_QUANT=none` (ignored with a warning otherwise). TBO ubatches keep the push combine, including ubatch 0, which shares all2all handle slot 0 with the synchronous path: the transport is chosen per `combine()` call. |
| **ATOM_MORI_TBO_HALF_BUFFERS** | bool | 0 (false) | If set to `1`, the second TBO ubatch MoRI op (all2all handle slot 1, created by `--enable-tbo` on the mori EP path next to slot 0, which serves both the sync path and ubatch 0) is sized for `ceil(max_num_batched_tokens / 2)` input tokens per rank instead of the full budget, halving its symmetric-heap footprint (about 4 GB to 2 GB at 16384 bf16 tokens on EP8; size `MORI_SHMEM_HEAP_SIZE` for the full slot-0 op plus the half-size slot-1 op). Sound because the token-midpoint prefill split never gives a ubatch more than half of a rank's tokens. Ignored, with a warning, when the split can be uneven: `--enable-tbo all`, PCP, or `ATOM_TBO_PREFILL_TOKEN_SPLIT=0`. A ubatch over its op's capacity is refused before dispatch rather than written past the buffers. |
