...
| **ATOM_ENABLE_METRICS_DEVICE_TIMER** | bool | 0 (false) | Collect target forward and cumulative request prefill GPU time with pooled CUDA/HIP events. At most 256 pending pairs per worker, FIFO polling, no GPU synchronization. Agentic CI enables this basic timer; sampling/propose require the separate device-stages switch. Read at worker initialization. |
| **ATOM_ENABLE_METRICS_DEVICE_STAGES** | bool | 0 (false) | Additionally collect GPU sampling and MTP propose durations. Requires `ATOM_ENABLE_METRICS_DEVICE_TIMER=1`. Disabled stages register no histograms and record/poll no additional events. Read at worker initialization. |
| **ATOM_ENABLE_METRICS_OUTPUT_DELIVERY** | bool | 0 (false) | Collect `atom:ttft_output_delivery_seconds`: scheduler first output to first generated SSE payload in the API generator. Set before starting both API and engine processes. Single-sequence streaming only; no client transport time. Cross-host timing requires synchronized wall clocks. |
...
| **ATOM_TTFT_TRACE** | bool | 0 (false) | Set to `1` before process startup to emit `record_function` spans at the decode TTFT stage boundaries: `ttft[api_preprocess]`, `ttft[prepare_model]`, `ttft[gpu_forward]`, `ttft[postprocess]`. Read once when the trace module loads; disabled call sites bypass trace context managers and do not reread the environment. Independent of `ATOM_TORCH_PROFILER_DIR`, but only visible in a captured trace. See `docs/ttft_breakdown_guide.md`. |
| **ATOM_API_PROFILER_DIR** | str | — | When set, `/start_profile` and `/stop_profile` additionally capture a CPU-only `torch.profiler` trace of the **API process** and write it to `<dir>/api_process_trace.json`. Honours `ATOM_PROFILER_MORE` for `record_shapes`/`with_stack`/`profile_memory`. Used to attribute `atom:ttft_api_preprocess_seconds` to tokenization rather than to waiting. A duplicate `/start_profile` is ignored with a warning rather than stranding the running profiler. |
