...
**Checkpoints need no separate allocation class.** A checkpoint *is* a group sitting on the free list with its content intact, indexed by the content hash of the last block it covers — the same lazy-eviction model the block pool uses, where hand-out (`StateGroupPool.pop`), not free, is the eviction event. A retained checkpoint therefore occupies one idle group, but the pool never protects that group from admission. With only the live floor, full concurrency can drain the checkpoint set; with explicit V4 headroom, up to the extra-group budget can remain while `max_num_seqs` requests are live.
...
**What the interval is pacing.** A checkpoint consumes an idle group from the already-sized STATE pool, and it also costs the request that takes it a forward: its prompt gets cut at the rung, and the extra forward is paid whether or not anyone ever resumes from it. That execution cost is the same under both mechanisms — a checkpoint holds the state as of the end of a forward, so the forward has to end on the boundary either way. What a copy adds on top is one contiguous device-to-device copy per checkpoint and per resume, which for the current DeepSeek-V4 layout is about 24.2 MiB per rank (compressor state plus sliding window). That is why the interval counts tokens rather than blocks, and why a prompt shorter than one interval checkpoints nothing at all — on a workload of short, mutually-distinct prompts the hit rate is 0 by construction, so the feature has to be free there. Measured on Qwen3.5-27B tp2 at ISL/OSL 1024/1024, checkpointing unconditionally at the last eligible boundary cost 17.5% of total throughput for zero resumes.
...
| `atom/config.py` | `Config` — scheduling-related fields (`max_num_seqs`, `max_num_batched_tokens`, `kv_cache_block_size`, `enable_prefix_caching`, state-checkpoint sizing, `scheduler_delay_factor`), `SpeculativeConfig` |
