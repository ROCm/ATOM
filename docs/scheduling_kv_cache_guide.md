...
**A store leaves a floor for live KV** (`BlockManager._checkpoint_reserve_units`). `_fresh_block` raises when the pool is dry and nothing is evictable, and a checkpoint is unevictable while it is `COPYING` or while a restore pins it. `schedule` publishes the previous batch's stores before it allocates anything, so `COPYING` never overlaps an allocation — but `allocate` pins a restore and then asks for fresh blocks *in the same pass*, and that pin holds until the next `complete_previous_state_batch`. A pass of prefix hits can therefore pin every checkpoint it resumes from and then find nothing to evict. The floor is one pass's worth of new blocks — `ceil(max_num_batched_tokens / block_size)` for a prefill chunk plus `max_num_seqs` appends — which is what keeps such a pass from ever having to evict, so how much of the cache it pins stops mattering. It is sized against the pass, not against the unevictable set. A store that would breach it is dropped and counted in `checkpoints_dropped`; the floor is only ever *tested*, never evicted towards, because eviction gives up only after it has emptied the cache and would otherwise destroy it on the way to refusing.

The ladder asks the same question before it acts. A demand is an instruction to cut a prefill chunk onto a rung, and that cut costs the request a forward, so `_record_checkpoint_demand` records one only while `has_room_for_store` holds — otherwise the forward is bought for a store the floor is about to refuse. What is *not* suppressed is the attribution: `num_wanted_hit_blocks` and hence `Lost-to-checkpoint` still say the reuse was declined for want of a checkpoint, because it was. `demands_declined_no_room` is where the difference shows, which keeps "the ladder is quiet because there is no demand" distinguishable from "the ladder is quiet because the pool is tight".
