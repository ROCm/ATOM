...
def test_deferred_decode_returns_the_flat_width_it_staged():
    ...
    processor.runner = SimpleNamespace(enforce_eager=True, graph_bs=[])
    ...
    with mock.patch(
        ...
        result = tokenIDProcessor.prepare_input_ids(processor, batch)

    assert result.shape == (8,)
    processor.input_ids.copy_to_gpu.assert_called_once_with(8)
