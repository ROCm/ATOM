...
# ── media prompts never consult the offload tier ───────────────────────────


class TestMultimodalSkipsConnectorLookup:
    """Temporary, same reason as the HBM cache: LMCache keys on the raw token
    ids, and media placeholders are the same token id whatever image they stand
    for. Guarded at the one place the scheduler asks.
    """

    def _sched(self, calls):
        sched = Scheduler.__new__(Scheduler)
        sched.kv_connector = SimpleNamespace(
            get_num_new_matched_tokens=lambda seq: (calls.append(seq.id), (8, False))[1]
        )
        return sched

    def test_text_prompts_still_ask(self, seq_factory):
        calls = []
        sched = self._sched(calls)
        seq = seq_factory(list(range(8)))

        assert sched._query_connector_prefill_match(seq, skip=False) is False
        assert calls == [seq.id]
        assert seq.offload_joint.kv_prefix_tokens == 8

    def test_media_prompts_do_not_ask(self, seq_factory):
        calls = []
        sched = self._sched(calls)
        seq = seq_factory(list(range(8)), multimodal_data={"pixel_values": object()})
        ...
        assert sched._query_connector_prefill_match(seq, skip=False) is False
        assert calls == []
        # And nothing was recorded as reachable in the remote tier.
        assert seq.offload_joint.kv_prefix_tokens == 0

    def test_the_marker_outlives_the_batch_dropping_multimodal_data(self, seq_factory):
        calls = []
        sched = self._sched(calls)
        seq = seq_factory(list(range(8)), multimodal_data={"pixel_values": object()})
        seq.multimodal_data = None  # what ScheduledBatch.__init__ does
