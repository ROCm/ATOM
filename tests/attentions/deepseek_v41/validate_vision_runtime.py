...
def main():
    ...
    report = {
        ...
        "tp": size,
        "cases": [],
        "scope": "native wo_a/mHC padding for 2..64 rows, independent of request state",
        "requests": args.requests,
        "sources": {
            name: hashlib.sha256(
                Path(__file__).resolve().with_name(name).read_bytes()
            ).hexdigest()
        ...
        def run(name, seqs, *, preempt=False, cancel=False, reorder=False):
            ...
            for tick in range(100):
                ...
                if (
                    ...
                    and first.num_cached_tokens >= 256
                ):
                    if preempt:
