...
def test_validate_dispatch_inputs_in_sync_and_drift():
    # The grid mirrors the prefixes with a random variant; agentic-only prefixes
    # (m3-mxfp4-agentic) are exempt -- they ride the dedicated agentic toggles.
    grid = catalog.grid_prefixes(CATALOG)
    assert catalog.validate_dispatch_inputs(CATALOG, grid) == []
    # missing a checkbox
    assert catalog.validate_dispatch_inputs(CATALOG, grid - {"glm-5-2-fp8"})
    # extra checkbox
    assert catalog.validate_dispatch_inputs(CATALOG, grid | {"ghost"})
    # an agentic-only prefix is NOT a grid checkbox
    assert "m3-mxfp4-agentic" not in grid


def test_workflow_dispatch_inputs_match_catalog():
    """The workflow_dispatch model-grid toggles must equal the grid prefixes."""
    ...
    assert model_toggles == catalog.grid_prefixes(CATALOG)


def test_agentic_model_inputs_cover_the_agentic_prefixes():
    """Each agentic prefix has one dedicated, reserved dispatch toggle that the
    workflow actually exposes -- so a new agentic model cannot be added without
    its own checkbox, and the toggles never leak into the random model grid."""
    from build_benchmark_matrix import AGENTIC_MODEL_INPUTS

    agentic_prefixes = {
        v["prefix"]
        for v in catalog.load_variants(CATALOG)
        if v["bench_kind"] == "aiperf_agentic"
    }
    assert set(AGENTIC_MODEL_INPUTS.values()) == agentic_prefixes
    assert set(AGENTIC_MODEL_INPUTS) <= RESERVED_INPUTS

    yaml = pytest.importorskip("yaml")
    wf = yaml.safe_load(WORKFLOW.read_text())
    on = wf.get("on", wf.get(True))
    dispatch_inputs = set(on["workflow_dispatch"]["inputs"])
    assert set(AGENTIC_MODEL_INPUTS) <= dispatch_inputs
...
def test_agentic_variants_carry_what_the_recipe_requires():
    ...
    """
    agentic = catalog.build_cells(CATALOG, bench_kind_filter={"aiperf_agentic"})
    assert agentic, "expected agentic cells"
    # Universal to every agentic variant, whatever the shared config defaults to:
    # the replay is multi-turn sessions, so a warm prefix cache is the workload.
    for cell in agentic:
        assert "--enable_prefix_caching" in cell["server_args"], cell["result_filename"]

    # DeepSeek-V4-Pro's config turns prefix caching OFF for every variant, so its
    # agentic variants re-enable it by appending the positive form; `build_args`
    # puts variant args after config args, so the negation must come first to be
    # overridden. (A model with its own agentic config, like M3, enables prefix
    # caching directly, with no negation to override.)
    ds = [c for c in agentic if c["prefix"] == "deepseek-v4-pro"]
    assert ds, "expected DeepSeek-V4-Pro agentic cells"
    for cell in ds:
    ...
    dpa = {c["conc"] for c in ds if "--enable-dp-attention" in c["server_args"]}
    plain = {c["conc"] for c in ds if "--enable-dp-attention" not in c["server_args"]}
    ...
    # MiniMax-M3-MXFP4 Agentic: EAGLE3-GQA speculative decode on a dedicated TP4
    # config (not MTP/tp8/DPA like DeepSeek), so its invariants are its own.
    m3 = [c for c in agentic if c["prefix"] == "m3-mxfp4-agentic"]
    assert m3, "expected MiniMax-M3-MXFP4 agentic cells"
    for cell in m3:
        args = cell["server_args"]
        for flag in (
            "--method eagle3",
...
def test_dp_agentic_variant_carries_the_session_routing_env():
    ...
    for cell in catalog.build_cells(CATALOG, bench_kind_filter={"aiperf_agentic"}):
        ...
        # InferenceX's dsv4_fp4_mi355x_atom_mtp.sh. DeepSeek-V4-Pro only -- other
        # agentic models pin their own accept target (M3: acceptance-rate).
        if cell["prefix"] == "deepseek-v4-pro":
