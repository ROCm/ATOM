...
def run(fn):
    ...
    fn(block_tables=block_tables, batch_id_per_token=batch_id_per_token,
       positions=positions, swa_indptr=swa_indptr, csa_indptr=csa_indptr,
       hca_indptr=hca_indptr, swa_indices=swa, csa_indices=csa, hca_indices=hca,
       T=T, win=win, block_size=block_size)
    return swa, csa, hca
...
phys = int(block_tables[1, 13 // block_size]); exp = phys * block_size + 13 % block_size
