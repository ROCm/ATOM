...
def export_and_check(prof, name, path):
    ...
    expected_count = 1 if name == "bf16_opus" else 2
    assert len(kernels) == expected_count, [(e["name"], e.get("dur")) for e in kernels]
    e = kernels[0]
    assert expected in e["name"], e["name"]
    assert e["args"]["grid"][:2] == [12, 1], e["args"]
    assert e["args"]["block"] == [512, 1, 1], e["args"]
    splits = e["args"]["grid"][2]
    if name == "bf16_opus":
        assert splits == 1
    else:
        assert splits > 1
        assert "flash_attn_splitk_combine" in kernels[1]["name"]
    return {
        "name": e["name"],
        "duration_us": sum(kernel["dur"] for kernel in kernels),
        "grid": e["args"]["grid"],
        "block": e["args"]["block"],
        "num_kv_splits": splits,
        "kernels": [
            {"name": kernel["name"], "duration_us": kernel["dur"]} for kernel in kernels
        ],
    }
