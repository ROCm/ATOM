...
def test_resumable_from_ignores_the_tier_while_loads_are_unwired(monkeypatch):
    ...
    pool = StateGroupPool(
        num_groups=2, transfer=StateTransfer.fork(1), hash_block_size=4
    )
    pool.offload = index()
...
def floored_pool(floor, monkeypatch):
    ...
    pool = StateGroupPool(
        num_groups=4, transfer=StateTransfer.fork(1), hash_block_size=4
    )
...
def test_an_orphan_is_spilled_only_when_kv_offload_can_bring_the_blocks_back(
    ...
    assert pool.lookup_group(555) == 0, "the orphan is the checkpoint on group 0"

    pool.unindex(555)
    assert pool.lookup_group(555) == -1
...
def spilling_pool(num_groups=4):
    pool = StateGroupPool(
        num_groups=num_groups, transfer=StateTransfer.fork(1), hash_block_size=4
    )
    pool.offload = StateOffloadIndex(staging_depth=2, kv_offload_enabled=False)
    return pool
