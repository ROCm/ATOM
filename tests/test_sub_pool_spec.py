...
class TestStatePool:
    ...
    def test_the_staging_ring_is_sized_here_not_by_the_caller(self, monkeypatch):
        """`state_pool` is the only reader of the ring size.
        ...
        spec = state_pool(ENTRY_STATE, 10, entries_per_req=3, offload_hosted=True)
    ...
    def test_the_checkpoint_headroom_and_the_staging_ring_add(self, monkeypatch):
        """Two fields, because the ring must not eat the backend's cushion.

        They shared one parameter once, and the ring overwrote it: the arena
        then lost the rows `state_entry_views(num_groups + slot)` addresses, and
        GDN's `cache[layer, lo : lo + span]` clamps rather than raising -- a
        short segment, an `entry_bytes` mismatch, and a crash in the packer.
        """
        monkeypatch.setenv("OFFLOAD_STATE_STAGING_GROUPS", "2")
        spec = state_pool(
            ENTRY_STATE, 10, entries_per_req=1, extra_entries=16, offload_hosted=True
        )
        assert spec.extra_entries == 16
        assert spec.staging_entries == 2

    def test_one_group_is_reserved_by_default(self, monkeypatch):
        """The tier has no on/off flag of its own -- it follows the
        `lmcache_offload` connector -- so the ring has to exist by default or
        the arena would be short the rows the spill path addresses. A
        deployment that will never offload sets the size to 0."""
        monkeypatch.delenv("OFFLOAD_STATE_STAGING_GROUPS", raising=False)
        spec = state_pool(ENTRY_STATE, 10, entries_per_req=3, offload_hosted=True)
        assert spec.staging_entries == 3

        monkeypatch.setenv("OFFLOAD_STATE_STAGING_GROUPS", "0")
        assert (
            state_pool(
                ENTRY_STATE, 10, entries_per_req=3, offload_hosted=True
            ).staging_entries
            == 0
        )
...
def test_checkpoint_headroom_is_allocated_and_admissible():
    ...
    hand out -- the backend's declared cushion. Withholding it
