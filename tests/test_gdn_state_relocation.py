...
def build(num_spec: int, replayssm: bool = False):
    ...
    span = 1 if replayssm else 1 + num_spec
    slots = GROUPS * span
...
def test_replayssm_group_is_a_single_slot():
    """Rollback is a cursor move, so there are no per-draft slots to carry."""
    stub, k, _, span = build(num_spec=7, replayssm=True)
    assert span == 1
    before_k = k.clone()
    ...
    assert torch.equal(k[:, 2], before_k[:, 2])  # neighbour untouched


def test_replayssm_records_and_cursor_travel_with_the_checkpoint():
    ...
    """
    stub, _, _, _ = build(num_spec=7, replayssm=True)
...
def test_baseline_relocation_does_not_look_for_replay_buffers():
    ...
    stub, k, _, _ = build(num_spec=2)
    assert not hasattr(stub.model_runner, "replayssm_buf_k")
    before_k = k.clone()

    GDNStateMixin.relocate_state_slots(stub, [(0, 2)])

    assert torch.equal(k[:, 2 * 3], before_k[:, 0])
