...
def _gather_backend(monkeypatch):
    ...
    monkeypatch.setattr(
        attention_mla, "gather_kv_b_proj_flydsl_supported", lambda *a, **k: True
    )
    monkeypatch.setattr(
        attention_mla,
...
def test_unsupported_gather_keeps_bf16_outputs_with_fp8_scales(monkeypatch):
    """The main-branch shape fallback must happen before FP8 workspace aliasing."""
    obj = _fake_self(True)
    k = torch.full((17, 12, 192), 9, dtype=torch.bfloat16)
    v = torch.full((17, 12, 128), 9, dtype=torch.bfloat16)
    asked = []
    ...
    monkeypatch.setattr(attention_mla, "gather_kv_b_proj_flydsl_supported", supported)
