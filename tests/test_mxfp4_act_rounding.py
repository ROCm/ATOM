...
# ── load real modules under lightweight stubs ─────────────────────────────────
# Populated by the module-scoped autouse fixture below. Lazy imports inside the
# function bodies (e.g. `from aiter.utility.mx_types ...`) execute at call time,
# so the stubs must stay in sys.modules while this module's tests run — but they
# are installed in a fixture (not at import time) and torn down afterwards, so
# nothing leaks into other test modules in the same session.

_linear = None
_utils = None
_resolve_mxfp4_act_round_mode = None
quant_mxfp4_online_even = None


@pytest.fixture(scope="module", autouse=True)
def _modules_under_test():
    global _linear, _utils, _resolve_mxfp4_act_round_mode, quant_mxfp4_online_even
    with _temporary_mocks():
        ...
        try:
            ...
            # Drop the mock-built modules we loaded by path; _temporary_mocks
            # only restores the names it patched, not these two.
            sys.modules.pop("atom.model_ops.linear", None)
            sys.modules.pop("atom.quantization.quark.utils", None)
            _linear = _utils = None
            _resolve_mxfp4_act_round_mode = quant_mxfp4_online_even = None
...
class TestResolveMxfp4ActRoundMode:
    ...
    def test_returns_even_by_default(self):
        assert _resolve_mxfp4_act_round_mode() == EVEN
    ...
    def test_env_var_forces_roundup(self, monkeypatch):
        ...
        assert _resolve_mxfp4_act_round_mode() == ROUND_UP

    def test_env_var_zero_stays_even(self, monkeypatch):
