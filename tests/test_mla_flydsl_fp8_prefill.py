...
softmax-scale forwarding, the descale ABI, and the AITER fallback -- not
kernel numerics.
"""
...
def test_undropped_rope_pad_falls_back():
    """The FP8 path requires the head dimension after dropping zero RoPE padding."""
...
@pytest.mark.parametrize(
    "scale", [_K3_SCALE, 1.8738542070926265 * _K3_SCALE, 0.5 * _K3_SCALE]
)
def test_softmax_scale_is_passed_without_changing_descales(scale, cpu_quantizer):
    ...
    assert fly.call_args.kwargs["softmax_scale"] == scale
    _, qs = cpu_quantizer(q)
    torch.testing.assert_close(fly.call_args.kwargs["q_descale"], qs, rtol=0, atol=0)
...
def test_fused_qkv_dispatch_preserves_descales(enabled, cpu_quantizer, monkeypatch):
    ...
    _, qs = cpu_quantizer(q)
    torch.testing.assert_close(fly.call_args.kwargs["q_descale"], qs, rtol=0, atol=0)
    assert fly.call_args.kwargs["softmax_scale"] == layer.scale
