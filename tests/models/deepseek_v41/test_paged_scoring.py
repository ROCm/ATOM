...
def test_paged_scoring_bands_match_one_shot(monkeypatch, candidate_mode, trim_tiles):
    ...
    rows, heads, dim, tile, units = 257, 32, 128, 256, 4
    width = tile * units
    ...
    actual = score_topk_paged(query, weights, plane, live_tiles, visible, **kwargs)
...
def test_unit_tiles_bound_prefill_but_keep_decode_capacity(decode):
    ...
    full = unit_table(tables, batches, 16)
    columns = 1024 if decode else 2
    # MQA_LOGITS_PRESHUFFLE_ROWS is 16: sixteen tiles per compressed page.
    assert actual.shape == (4, columns * 16)
    torch.testing.assert_close(actual, full[:, : columns * 16])
