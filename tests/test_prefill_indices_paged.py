...
csa_head = np.array([2,1,0,3,2, 1,0,2,1])  # arbitrary CSA topk head sizes (not written here)

def indptr(counts):
    v = np.zeros(T+1, np.int64); v[1:] = np.cumsum(counts); return torch.tensor(v, dtype=torch.int32, device=dev)
...
def run(fn):
    ...
    fn(positions=positions, bid_per_token=bid_per_token, chunk_start_per_seq=chunk_start,
       cu_seqlens_q_per_seq=cu_q, state_slot_per_seq=state_slot,
       n_committed_hca_per_seq=n_hca_per_seq, block_tables=block_tables, swa_block_tables=swa_block_tables,
       extend_indptr=ext_indptr, prefix_swa_indptr=swa_indptr,
       prefix_csa_indptr=csa_indptr, prefix_hca_indptr=hca_indptr,
       extend_indices=ext, prefix_swa_indices=swa, prefix_csa_indices=csa,
       prefix_hca_indices=hca, T=T, win=win, block_size=block_size,
       swa_pages=10_000, hca_ratio=hca_ratio)
...
print(f"PASS: prefill-indices kernel == reference (prefix_swa_count={prefix_swa_count.tolist()})")

# Spot: seq1 first token pos=16, global_pos starts swa_low=9 -> blk 9//8=1,
# phys=block_tables[1,1]; paged=phys*8 + 9%8.
phys = int(swa_block_tables[1, 9//block_size]); exp = phys*block_size + 9%block_size
