...
rate, not GSM8K.
  ...
  NOT TRACED, safe to edit and instrument: precompute_context_kv (deliberately
...
class _DSparkInner(nn.Module):
    ...
    ``forward_spec`` / ``precompute_context_kv`` signatures (the proposer's hot
    path is unchanged) and so ``precompute_context_kv`` stays eager: the
