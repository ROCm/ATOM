...
# HiSparse (DSA decode HBM offload) side-channel. When enabled the coordinator
...
# None (default) => HiSparse off, one branch-not-taken per indexer call, no cost.
...
_HISPARSE_TOPK_LOGICAL: torch.Tensor | None = None


def set_hisparse_topk_buffer(buf: torch.Tensor | None) -> None:
    """Register (or clear) the HiSparse logical-top-k side-channel buffer."""
    global _HISPARSE_TOPK_LOGICAL
    _HISPARSE_TOPK_LOGICAL = buf
...
def sparse_attn_indexer(
    ...
    if context.is_prefill:
        ...
        if _HISPARSE_TOPK_LOGICAL is not None:
            ...
            _HISPARSE_TOPK_LOGICAL[:n].copy_(topk_indices_decode)
