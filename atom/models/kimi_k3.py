...
class KimiSparseMoeBlock(nn.Module):
    def __init__(
        ...
        if self._use_dual_stream:
            # Register self so `maybe_dual_stream_moe_pair` can look this module
            # up by prefix from static_forward_context (the op is Dynamo-opaque).
            cc = get_current_atom_config().compilation_config
            cc.static_forward_context[self.prefix] = self

    def defers_shared_add(self) -> bool:
        """Whether forward() hands the shared-expert output back separately.

        Config-determined, so it is the same for the single- and dual-stream
        paths and a fake impl can decide it without running either.
        """
        if self.shared_experts is None:
            return False
        # Non-latent with TP has to sum before its single deferred all-reduce.
        return self.use_latent_moe or self.tp_size == 1
    ...
    def forward(
        ...
        """
        if self._use_dual_stream:
            # K3 needs both tensors back, so it cannot share V2/V4's
            # single-tensor dispatch op; same opaque-lookup trick, list schema.
            out = torch.ops.aiter.maybe_dual_stream_moe_pair(hidden_states, self.prefix)
            # Length is config-determined, so read it from the same predicate
            # the op's fake impl uses rather than from the returned list.
            return (out[0], out[1]) if self.defers_shared_add() else (out[0], None)
    ...
    def dual_stream_moe_forward(
        ...
        if self.use_latent_moe:
            ...
            if self.tp_size == 1:
                current.wait_stream(alt)
            # Handed out unsummed, so it outlives this call on `current`; the
            # allocator has to know before alt's block can be reused.
