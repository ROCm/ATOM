...
class Indexer(nn.Module):
    ...
    def _score_topk_prefill(
        ...
        # The logits kernel returns global committed-K indices.  Current main
        # uses the per-token sequence base as the matching lower bound; keep
        # that semantic when selecting only this rank's cyclic row shard.
        seq_base_all = indexer_meta["seq_base_per_token_gpu"]
        cu_starts = seq_base_all
