...
import torch
from aiter.dist.communication_op import tensor_model_parallel_all_reduce, tensor_model_parallel_fused_qknorm_allreduce
    ...
    def forward(
        ...
        if self.use_qk_norm:
            ...
            if qkv.shape[0] <= 64 and self.tp_size > 1:
                q, k, v = tensor_model_parallel_fused_qknorm_allreduce(qkv, self.q_norm.weight, self.k_norm.weight, self.rms_norm_eps)
            else:
