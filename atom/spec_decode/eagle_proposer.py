...
class EagleProposer(Drafter):
    ...
    def propose(
        ...
        elif attn_metadata.slot_mapping.shape[0] > len(input_ids):
            ...
            with record_function(f"draft[{i}/{self.mtp_k} bs={bs}]"):
                ...
                if i < self.mtp_k - 1:
                    ...
                    if has_flat_kv and "slot_mapping" not in workinfos:
                        ...
                        last = kv_indices[kv_indptr[1 : bs + 1] - 1]
                        mla_block_size = self.runner.attn_metadata_builder.block_size
                        if target_uses_mla and mla_block_size > 1:
                            ...
                            slot_mapping[:] = last * mla_block_size + (
                                attn_metadata.kv_last_page_lens[:bs] - 1
                            )
                        else:
                            # block_size==1: kv_indices already holds token slots.
                            slot_mapping[:] = last
