...
def decompose_slots_kernel(
    ...
    # Widen before the multiply: `program_id` and `arange` are i32, so widening
    # the product would only sign-extend an index that already wrapped.
    offs = tl.program_id(0).to(tl.int64) * TILE + tl.arange(0, TILE)
    # A shift and a mask rather than `//` and `%`: triton's integer division
    # truncates toward zero where torch's floors, but an arithmetic shift
    # floors, so a negative slot decomposes exactly as `slots // block` and
    # `slots % block` do.
    mask = offs < n_slots
...
def decompose_slots_triton(
    ...
    """Split int32 `slots` and `arange(n_iota)` into page, row and `row_lut[row]`.

    Returns six int32 tensors: the three for `slots`, then the three for the
    iota. One launch in place of the twelve divides, remainders and lookups plus
    the `arange` it takes in eager torch. `row_lut` maps a row within a block to
    wherever a second plane stores that row. Taking it as a table rather than a
    formula keeps the caller's own function the single definition of that bend.

    `n_slots` and `n_iota` are not specialized on, so one compiled variant
    serves every shape. Call it once at startup to keep the compile out of a
    served request.
    """
