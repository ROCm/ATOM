...
"""SparseKV swap-in kernels — thin shim over the aiter ``module_sparsekv_swap`` op.

The fused decode hot path (miss-detect + LRU evict + swap + translate) and the
new-token backup live in aiter (``aiter/csrc/py_itfs_cu/sparsekv_swap_kernels.cu``),
...
def _aiter():
    """Return the aiter sparsekv ops module, or None if aiter is unavailable."""
    try:
        import aiter
...
__global__ void sparsekv_gather_kernel(
...
#include <cstdint>
int64_t host_get_device_pointer(at::Tensor pinned_host_tensor);
void sparsekv_swap_in(int64_t host_cache_dev_ptr, at::Tensor device_buffer,
                      at::Tensor src_locs, at::Tensor dst_locs,
                      int64_t item_size_bytes);
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("host_get_device_pointer", &host_get_device_pointer);
  m.def("sparsekv_swap_in", &sparsekv_swap_in);
}
"""
...
def _fallback():
    ...
    _FALLBACK_MODULE = load_inline(
        name="atom_sparsekv_swap_fallback",
...
def host_get_device_pointer(pinned_host_tensor: torch.Tensor) -> int:
    ...
    a = _aiter()
    if a is not None:
        return a.sparsekv_host_get_device_pointer(pinned_host_tensor)
...
def sparsekv_swap_and_translate(
    ...
    if a is None:
        raise RuntimeError(
            "sparsekv_swap_and_translate requires the aiter module_sparsekv_swap "
            "op; aiter could not be imported."
        )
    a.sparsekv_swap_and_translate(
        cold_pool_dev_ptr,
        hot_buffer,
        topk_logical,
        indptr,
...
def sparsekv_swap_and_translate_record(
    ...
    if a is None:
        raise RuntimeError(
            "sparsekv_swap_and_translate_record requires the aiter "
            "module_sparsekv_swap op; aiter could not be imported."
        )
    a.sparsekv_swap_and_translate_record(
        cold_pool_dev_ptr,
        hot_buffer,
        topk_logical,
        indptr,
...
def sparsekv_copy_planned(
    ...
    if a is None:
        raise RuntimeError(
            "sparsekv_copy_planned requires the aiter module_sparsekv_swap "
            "op; aiter could not be imported."
        )
    a.sparsekv_copy_planned(
        cold_pool_dev_ptr,
        hot_buffer,
        req_slots,
        plan_miss_tok,
...
def sparsekv_backup_into_assigned(
    ...
    if a is None:
        raise RuntimeError(
            "sparsekv_backup_into_assigned requires the aiter module_sparsekv_swap "
            "op; aiter could not be imported."
        )
...
def sparsekv_backup_new_token(
    ...
    if a is None:
        raise RuntimeError(
            "sparsekv_backup_new_token requires the aiter module_sparsekv_swap "
            "op; aiter could not be imported."
        )
