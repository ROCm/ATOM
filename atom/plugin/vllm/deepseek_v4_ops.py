...
@triton.jit
def _v4_prefill_indices_fused_kernel(
    positions_ptr,
    bid_per_token_ptr,
    chunk_start_per_seq_ptr,
    cu_seqlens_q_per_seq_ptr,
    state_slot_per_seq_ptr,
    n_committed_hca_per_seq_ptr,
    block_tables_ptr,
    bt_stride_bs,
    extend_indptr_ptr,
    ...
    swa_pages,
    win: tl.constexpr,
    HCA_RATIO: tl.constexpr,
    BLOCK_N: tl.constexpr,
):
    """Build vLLM proxy-layout prefill indices in one program per token."""
    t = tl.program_id(0)
    ...
    n_hca = tl.minimum(
        (pos + 1) // HCA_RATIO,
        tl.load(n_committed_hca_per_seq_ptr + bid),
    )
...
def write_v4_prefill_indices_fused(
    *,
    ...
    """Build indices for vLLM's per-layer ``SWA || compressed`` proxy pool."""
    if T == 0:
        return
    ...
    for indptr in (
        ...
        assert indptr.dim() == 1 and indptr.shape[0] >= T + 1
    for indices in (
        extend_indices,
        prefix_swa_indices,
        prefix_csa_indices,
    ...
    _v4_prefill_indices_fused_kernel[(T,)](
        positions,
        bid_per_token,
        chunk_start_per_seq,
