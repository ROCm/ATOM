...
from atom.plugin.vllm.models.minimax_m3.amd.ops import (
    gemma_fused_add_rmsnorm,
    gemma_rmsnorm,
    swiglu_oai_split,
)
from atom.plugin.vllm.models.minimax_m3.common.indexer import MiniMaxM3Indexer
from atom.plugin.vllm.models.minimax_m3.common.mm_preprocess import (
    MiniMaxM3VLDummyInputsBuilder,
    MiniMaxM3VLMultiModalProcessor,
    MiniMaxM3VLProcessingInfo,
)
from atom.plugin.vllm.models.minimax_m3.common.sparse_attention import (
    MiniMaxM3SparseBackend,
    MiniMaxM3SparseImpl,
    select_main_impl_cls,
)
from atom.plugin.vllm.models.minimax_m3.common.vision_tower import MiniMaxVLVisionModel
...
class MiniMaxM3Attention(nn.Module):
    ...
    def __init__(
        ...
        self.attn = Attention(
            ...
            # ATOM's MXFP8 QuantizationConfig is for the projections; vLLM's
            # Attention KV-cache quant hook expects a different quant interface.
            quant_config=None,
...
class MiniMaxM3SparseAttention(nn.Module, AttentionLayerBase):
    ...
    def _page128_kv_cache_for_insert(self, kv_cache: torch.Tensor):
        num_blocks, _kv, block_size, num_kv_heads, head_dim = kv_cache.shape
        if self.kv_cache_dtype == "fp8":
            ...
            if self.k_scale is None or self.v_scale is None:
                self.k_scale = torch.ones(
                    (num_kv_heads, num_blocks * block_size),
                    dtype=dtypes.fp32,
        ...
        # Community MiniMax-M3 sparse attention uses the standard vLLM sparse
        # KV layout [num_blocks, 2, block_size, num_kv_heads, head_dim].
        # aiter.fused_qknorm_idxrqknorm supports that layout with
        # asm_layout=False and separate K/V slices.
        key_cache = kv_cache[:, 0]
        value_cache = kv_cache[:, 1]
        k_scale = self.k_scale if self.k_scale is not None else None
        v_scale = self.v_scale if self.v_scale is not None else None
    ...
    def forward(
        ...
        if hasattr(ops, "fused_minimax_m3_qknorm_rope_kv_insert"):
            ...
            k_cache, v_cache, k_scale, v_scale = self._page128_kv_cache_for_insert(
                self.kv_cache
            )
...
class MiniMaxM3SparseForCausalLM(nn.Module, SupportsPP, SupportsEagle3):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        try:
            ...
            from atom.plugin.vllm.model_wrapper import _prepare_env

            atom_config = generate_atom_config_for_plugin_mode(vllm_config)
            atom_config.hf_config = vllm_config.model_config.hf_config
            _prepare_env(atom_config=atom_config)
        if not getattr(atom_config.quant_config, "_minimax_m3_community_remapped", False):
            atom_config.quant_config.remap_layer_name(
                vllm_config.model_config.hf_text_config,
                packed_modules_mapping=self.packed_modules_mapping,
            )
            atom_config.quant_config._minimax_m3_community_remapped = True
        config = vllm_config.model_config.hf_text_config
...
class MiniMaxM3SparseForConditionalGeneration(
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        self.language_model_only = all(
            limit == 0 for limit in self.multimodal_config.limit_per_prompt.values()
        )
