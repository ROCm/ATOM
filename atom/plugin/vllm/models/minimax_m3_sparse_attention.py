...
class MiniMaxM3SparseImpl(AttentionImplBase[MiniMaxM3SparseMetadata]):
    ...
    def forward(
        ...
        if (
            main_md.num_actual_tokens != index_md.num_actual_tokens
            or main_md.num_decode_tokens != index_md.num_decode_tokens
            or main_md.num_prefill_tokens != index_md.num_prefill_tokens
            or main_md.num_decodes != index_md.num_decodes
            or main_md.num_prefills != index_md.num_prefills
        ):
            raise RuntimeError(
                "MiniMax-M3 main/index sparse metadata split mismatch: "
                f"main(tokens={main_md.num_actual_tokens}, "
                f"decodes={main_md.num_decodes}/{main_md.num_decode_tokens}, "
                f"prefills={main_md.num_prefills}/{main_md.num_prefill_tokens}) vs "
                f"index(tokens={index_md.num_actual_tokens}, "
                f"decodes={index_md.num_decodes}/{index_md.num_decode_tokens}, "
                f"prefills={index_md.num_prefills}/{index_md.num_prefill_tokens})."
            )
