...
class MiniMaxVLPatchEmbed(nn.Module):
    ...
    Takes flat tokens of shape (N, C * temporal_patch_size * patch_size^2)
    ...
    def forward(self, pixel_values: torch.Tensor) -> torch.Tensor:
        # pixel_values: (N, C * temporal_patch_size * patch_size^2)
...
class MiniMaxVLVisionTransformer(nn.Module):
    ...
    def forward(
        ...
        # pixel_values: (total_N, C * temporal_patch_size * patch_size^2)
...
class MiniMaxVLPatchMerger(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x: (N, text_hidden_size) -> (N // merge_size^2, text_hidden_size)
