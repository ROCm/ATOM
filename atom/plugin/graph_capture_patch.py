...
def _iter_aiter_capture_ca_comms():
    """Yield the aiter ca_comm(s) that must enter graph-capture mode.

    Reuse-TP-as-CP intentionally aliases ``_PCP`` to TP for activation
    collectives, so those two getters normally resolve to the same ca_comm and
    are deduplicated here. Weight prefetch uses a separate raw process group and
    does not participate in aiter ca_comm capture.
    """
    getters = []
    try:
