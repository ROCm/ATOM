...
"""Triton byte gather/scatter for ATOM KV offload — two layouts, one module.
...
* **Chunk-major layout** (``fused_pack_chunk_major`` / ``fused_unpack_chunk_major``)
  — the original token-chunked MHA/MLA path, where every segment shares ONE
  ``block_ids`` list and the block lookup is computed inside the kernel (no
  per-item host list). Cheapest for the many-small-blocks-per-chunk case.

* **Region layout** (``gather_regions`` / ``scatter_regions``) — the general
  primitive added for the DSV4 offload unit. Each :class:`GatherRegion` is one
  source region (base addr + per-block ``unit_bytes``) with its own physical-id
  list and destination byte offset; blocks land contiguously. Handles components
  with different strides / id lists (compressed / SWA / CSA state).

Both share ``_BLOCK_BYTES`` and the int64 plan helper ``_device_i64``.
"""
...
# =====================================================================
# Region layout — general gather/scatter primitive (DSV4 offload unit)
# =====================================================================
class _NullCtx:
    def __enter__(self):
        return None
