...
"""Pure key, fingerprint, and commit-index policy for PAGE+SLOT offload."""
...
def sidecar_boundary_tokens(
    *,
    ...
    """Return regular and terminal PAGE+SLOT checkpoints in sorted order."""
    ...
    boundaries = {terminal}
    if sidecar_interval > 0:
        boundaries.update(range(sidecar_interval, terminal + 1, sidecar_interval))
    return tuple(
        boundary
...
def select_pending_sidecar_boundary(
    ...
    *,
    ...
    # One request has one live SLOT source. While an earlier snapshot is being
    # published, the scheduler must pause this partial prefill instead of
    # selecting a later boundary that cannot yet be snapshotted.
