...
* **ATOM-owned raw-byte GPU connector** — LMCache's stock vLLM GPU connectors
  ...
  :class:`ATOMKVByteCodec`, or page-major DSV4 ``block_regions`` with
...
def _validated_kv_role(kvc: dict) -> str:
...
class LMCacheOffloadConnector(KVConnectorBase):
    ...
    def __init__(self, config) -> None:
        ...
        self.kv_role = _validated_kv_role(kvc)
        ...
        self._engine = None
        self._codec: ATOMBlockByteCodec | None = None
        self._lookup_server = None
        self._slot_codec: ATOMSlotSidecarCodec | None = None
        self._slot_store: SlotSidecarStore | None = None
    ...
    def register_kv_caches(
        ...
        if not kv_caches and block_regions:
            ...
            self._codec = ATOMPageRegionCodec(
                block_regions,
        ...
        if isinstance(self._codec, ATOMPageRegionCodec) and stateful_page:
    ...
    def _finish_unadmitted_save(self, req: LMCacheReqMeta) -> None:
        ...
        logger.warning(
            "LMCache offload: save rejected req=%s reason=max_pending_saves "
            "capacity=%d",
            req.req_id,
    ...
    def _copy_slot_staging_to_cpu(self, staging_id: int) -> torch.Tensor:
        slot_codec, _, _, _ = self._require_slot_components()
        row = slot_codec.staging_view(staging_id)
        host = torch.empty(
            (HEADER_BYTES + slot_codec.payload_bytes,),
            dtype=torch.uint8,
            device="cpu",
    ...
    def _restore_slot_payload(
        ...
        if int(host.numel()) != int(slot_codec.payload_bytes):
            raise ValueError(
                "SLOT payload size changed after decode: "
                f"got {int(host.numel())}, expected {slot_codec.payload_bytes}"
            )
        row = slot_codec.staging_view(staging_id)
        stream = self._create_slot_stream()
        try:
...
class LMCacheOffloadConnectorScheduler(KVConnectorSchedulerBase):
    ...
    def __init__(self, config) -> None:
        ...
        # Keep the public scheduler statistics added on main while tracking
        # exact operation generations for correctness.
        ...
        try:
            ...
            self.chunk_size = int(cfg.chunk_size)
            self.resume_alignment = lcm(self.chunk_size, self.hash_block_size)
            self.sidecar_interval = (
                lcm(checkpoint_interval, self.resume_alignment)
    ...
    def _sidecar_hash_data(self, seq) -> tuple[dict[int, int], list[tuple[int, int]]]:
        ...
        if alignment <= 0:
            ...
            self._sidecar_hash_cache[sid] = (seq, *result)
            return result
        num_prompt = int(getattr(seq, "num_prompt_tokens", 0))
        terminal = (num_prompt // alignment) * alignment
        if terminal <= 0:
            result = ({}, [])
            self._sidecar_hash_cache[sid] = (seq, *result)
            return result
        ...
        boundaries = {terminal}
        interval = int(getattr(self, "sidecar_interval", 0) or 0)
        if interval > 0:
            boundaries.update(range(interval, terminal + 1, interval))
        boundaries = {
            boundary
            for boundary in boundaries
            if boundary > 0 and boundary % alignment == 0
        }
        if not boundaries:
            result = ({}, [])
            self._sidecar_hash_cache[sid] = (seq, *result)
    ...
    def _next_pending_sidecar_boundary(
        ...
        if (
            ...
            return None
        inflight = self._sidecar_save_inflight.get(str(seq.id))
        for boundary, boundary_hash in self._sidecar_boundary_records(seq):
            identity = (boundary, boundary_hash)
            if not start < boundary <= end:
                continue
            if (
                boundary_hash in self._committed_sidecar_hashes
                or (inflight is not None and identity == inflight[1:])
                or identity in self._failed_sidecar_saves.get(str(seq.id), set())
            ):
                continue
            return identity
        return None
