...
Send/save lifetime
------------------
A completion report names a finished operation; it is not permission to free a
request. That is a separate question, answered by ``should_defer_free``, which
this composite ORs across its subs -- an in-flight P/D send and an offload save
(or a computed chunk with no save dispatched yet) each veto the free alone.

The separation is load-bearing. Offload keeps at most one save per request in
flight, so chunk *k*'s ``finished_saving`` is what dispatches chunk *k+1*, and
mooncake only sends after the **last** chunk: withholding the save until the
send arrives deadlocks the two and drops every chunk after the first.
"""
