...
class SuperblockSource(Protocol):
    """The three calls a state pool makes when its slots come from the KV pool.

    Deliberately these three and no more, so the pool cannot reach past them
    into the block layer: a state slot is one contiguous superblock, taken and
    given back, and nothing here knows about blocks, hashes or ref counts.
    `BlockPool` satisfies this.
    """
...
class StateSlotPool:
    ...
    def __init__(
        ...
        # Where a slot's bytes come from, when they are not a statically carved
        # region. With a source, `num_slots` is a ceiling rather than a
        # reservation: a slot is one superblock taken from the paged pool at
        # admission and given back when the request ends, so the split between
        # KV and state is whatever the workload asks for. Without one, every
        # slot exists from startup and the two pools are carved apart, which is
        # the behaviour every non-hybrid model still gets.
        self._supers = superblock_source
        # The hard ceiling on how many slot indices may ever exist: the slot
        # dimension of the runner's state tensor. A slot index addresses
        # `mamba_k_cache[layer, slot]`, allocated once at startup, so an index
        # past this is an out-of-bounds GPU read rather than a refused
        ...
        # Read side. Every counter above records what became of a checkpoint
        # this pool WROTE; none says whether anything ever read one, so "kept
        # 798" has been unfalsifiable as a claim about value. `resumed` counts
        # resume events, `_read_count` splits them per slot. The trace says 87%
        # of anchors are read exactly once and 7% never; these say whether the
        # served workload agrees.
        self.checkpoints_resumed: int = 0
        self._read_count: dict[int, int] = {}
        # A later anchor of the same chain covers this one's prefix, so a
        # right-to-left `resumable_hit` scan reaches the deeper one first. NOT
        # proof the older one is dead — a turn whose hit lands between the two
        # falls back to it, which the trace puts at 16.5% of resumes. So this
        # counts and marks only. Nothing spends `_superseded` yet: sizing a
    ...
    def has_free(self, count: int = 1) -> bool:
        ...
        With a superblock source, a slot index is not enough: the bytes behind
        it have to be obtainable too, and a slot already backed needs nothing
        further. So the question is asked of the source only for the shortfall
        — the slots that would have to be freshly backed — and answered against
        the paged pool, which may have to evict a superblock to say yes.

        And the index space is no longer the limit it is without a source.
        `num_slots` there is `max_num_seqs * entries_per_req + extra_entries`,
        computed before the pool existed, so it caps checkpoints at roughly the
        concurrency however much room the pool has — which is the static carve
        this design set out to remove, surviving in index form. With a source
        the free list can grow: a shortfall of indices is a question for the
        paged pool, exactly as a shortfall of bytes is.
        """
    ...
    def _can_mint(self, count: int = 1) -> bool:
        ...
        `max_slots` is the hard one: a slot index addresses `mamba_k_cache[
        layer, slot]`, a tensor the runner allocates once at startup, so an
        index past its slot dimension is an out-of-bounds GPU read. That is
        not a refused allocation — it is `hipErrorLaunchFailure`, eleven
        minutes into a benchmark, with the traceback pointing at a KDA kernel
        rather than at the allocator. Sizing that tensor for the superblock
        supply instead is not open either: 627 slots is 32.8 GiB at Kimi-K3's
        geometry, which is the entire paged pool.

        So the superblock supply governs the *bytes*, and the state tensor
        governs *how many slots can exist at all*. Growth stops at whichever
        runs out first, and on this model it is the tensor.
        """
        if count <= 0 or self._supers is None:
            return False
        if self.max_slots and self.num_slots + count > self.max_slots:
            return False
        return self._supers.can_claim_superblocks(count)
    ...
    def pop(self) -> int:
        ...
        if slot < 0 and self._can_mint():
            ...
            # out of a supply the pool sizes itself from, where spending a
            # checkpoint costs a resume that would have hit. Between those two
            # the superblock is almost always cheaper.
            #
            # Gated on `_pop_vacant` returning nothing, NOT on `_free` being
            # empty: `_free` holds checkpointed slots too, so a pool with any
            # checkpoint resident never looked empty and this never ran. It
            # evicted 114 checkpoints against 627 free superblocks before the
            # `[Superblocks]` line made that visible.
            self.extend(1)
            slot = self._pop_vacant()
        if slot < 0:
        ...
        self._ensure_backed(slot)
        return slot

    def _ensure_backed(self, slot: int) -> None:
        """Give `slot` a superblock to live in, if it has not got one.

        A slot that already carries one keeps it: re-backing would hand its
        bytes back and take different ones for no gain, and the copy of the
        state that a resuming request is about to read lives in those bytes.

        `has_free` is the gate and was asked first, so a refusal here means the
        paged pool moved between the two — which `AssertionError` is right for,
        the same way `_fresh_block` treats a PAGE unit it was promised.
        """
        if self._supers is None or slot in self._slot_super:
            return
        index = self._supers.claim_superblock()
        if index < 0:
            raise AssertionError(f"no superblock available to back state slot {slot}")
        self._slot_super[slot] = index

    def _unback(self, slot: int) -> None:
        """Give a vacant slot's superblock back to the paged pool.

        Only ever from `release` of a slot carrying nothing. A slot holding a
        checkpoint keeps its backing until something spends the checkpoint,
        because the checkpoint *is* those bytes.
        """
        if self._supers is None:
            return
        index = self._slot_super.pop(slot, -1)
        if index >= 0:
            self._supers.release_superblock(index)
    ...
    def promote(self, slot: int) -> None:
        ...
        if not self.enabled:
            return
        # Counted before the `_speculative` gate below, not after: with the
        # demand rung off every checkpoint is a prompt-end anchor and none is
        # speculative, so a counter behind that gate would read zero for the
        # exact configuration being measured. This method is the resume site —
        # `claim` is not, because `_set_hash` calls it to re-file a slot
        # nobody read.
        self.checkpoints_resumed += 1
        self._read_count[slot] = self._read_count.get(slot, 0) + 1
        if slot not in self._speculative:
            return
        self._speculative.discard(slot)
        if slot in self._free and self.slot_hash[slot] != -1:
            self._checkpointed.remove(slot)
    ...
    def release(self, slot: int) -> None:
        ...
        if self.slot_hash[slot] != -1:
            if slot in self._speculative:
                ...
                self._checkpointed.append(slot)
            # Keeps its superblock: the checkpoint *is* those bytes, and a
            # resuming request reads them in place. Handing the backing back
            # here would leave an index entry pointing at memory the paged pool
            # had already reused — findable, and wrong.
            return
        self._speculative.discard(slot)
        # Vacant, so the bytes hold nothing anyone can reach. This is where the
        # carve becomes dynamic: capacity a request no longer needs goes back
        # to KV rather than sitting reserved until the process ends.
        self._unback(slot)
    ...
    def checkpoint(self, seq, boundary_blocks: int, h: int) -> None:
        ...
        if (
            prev_anchor >= 0
            and self.slot_hash[prev_anchor] != -1
            and prev_anchor not in self._superseded
        ):
    ...
    def checkpoint_fates(self) -> dict[str, int]:
        ...
        """
        read_twice = sum(1 for n in self._read_count.values() if n >= 2)
        return {
            ...
            # Cumulative resume events, and how many of the checkpoints still
            # resident have served more than one. `resumed` over `kept` is the
            # first measurement of whether writing a checkpoint was worth the
            # slot; `read_twice` tests the claim that reuse is one-shot, which
            # decides whether spending a checkpoint on its first read is safe.
            "checkpoints_resumed": self.checkpoints_resumed,
            "checkpoints_read_twice": read_twice,
            # Gauge, not a counter: how many resident checkpoints a deeper
            # anchor of the same chain now covers. This is the candidate set a
            # soft cap would spend, so its size decides whether a soft cap has
            # anything to do.
    ...
    def count_unreachable(self, block_index_has) -> int:
        """Held checkpoints whose hash is no longer in the block index.

        `unindex` fires only for the hash of the block that was evicted, so a
        checkpoint whose prefix died at an *earlier* block keeps its hash, keeps
        its slot, and keeps its superblock — which `SuperblockMap.is_reclaimable`
        then exempts from paged eviction. It is dead, invisible, and holding a
        superblock the KV pool is evicting to do without.

        O(held) and walks no chains, so it is a diagnostic rather than something
        for the allocation path. What it measures is whether paying for the
        exact version would be worth it.
        """
        return sum(
            1
            for slot in self._free
            if self.slot_hash[slot] != -1 and not block_index_has(self.slot_hash[slot])
        )
    ...
    def invalidate(self, slot: int) -> None:
        ...
        # Both marks describe the checkpoint, not the slot. The slot index is
        # reused for an unrelated checkpoint later, so carrying either across
        # would report the new occupant as superseded, or as already read, on
        # the strength of what the old one did.
        self._superseded.discard(slot)
        self._read_count.pop(slot, None)
