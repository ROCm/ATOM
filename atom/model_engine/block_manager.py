...
class BlockManager:
    def __init__(
        ...
        *,
        ...
        # Superblocks, when the sizing plan says a state slot is a whole number
        # of paged blocks. That makes the KV/state split dynamic: a slot is one
        # superblock taken at admission and given back when the request ends,
        # rather than a statically carved region that idles when the workload
        # does not want it. `None` — every model whose plan says nothing —
        # leaves `BlockPool` behaving exactly as it did before superblocks.
        blocks_per_super = int(getattr(config, "blocks_per_superblock", 0) or 0)
        superblocks = None
        if blocks_per_super > 1:
            # Round the pool down to a whole number of superblocks rather than
            # falling back. The remainder is under one superblock -- 31 blocks
            # of 18035 at Kimi-K3 -- and giving it up costs 0.17% of the pool,
            # where refusing to group would silently leave every model whose
            # sizing does not happen to divide with no superblocks at all. A
            # feature that quietly does nothing on the model it was built for
            # is worse than one that costs a rounding error.
            grouped = (num_blocks // blocks_per_super) * blocks_per_super
            if grouped:
                if grouped != num_blocks:
                    logger.info(
                        "[Superblocks] %d blocks -> %d (%d x %d); %d dropped to "
                        "make whole superblocks",
                        num_blocks,
                        grouped,
                        grouped // blocks_per_super,
                        blocks_per_super,
                        num_blocks - grouped,
                    )
                num_blocks = grouped
                superblocks = SuperblockMap(num_blocks, blocks_per_super)
        self.kv = BlockPool(
            num_blocks, on_evict=self._record_evicted, superblocks=superblocks
        )
        ...
        # How far the slot pool may grow, which is a different question from
        # how many it starts with. A slot index addresses the runner's state
        # tensor, so the ceiling is that tensor's slot dimension -- and which
        # number that is depends on how it was allocated. The runner reports
        # it; deriving it here would be deriving it twice.
        ...
        #
        # Built before the PAGE coordinator because that coordinator borrows
        # this pool as its staging allocator. When it exists this pool is handed
        # `none()` and keeps only its allocator half — the checkpoint index has
        ...
        self.state = StateSlotPool(
            ...
            # With superblocks the bytes behind a slot come from the paged pool
        ...
        if checkpoint_spec is not None:
            ...
            self.paged_state_checkpoints = PagedStateCheckpointCoordinator(
                ...
                checkpoint_spec,
                enabled=enabled,
                # The capability travels with the backend, not with the storage:
                # a chunk-kernel backend can hand over an interior state whether
                # that state then lands in a slot or in PAGE units.
                readable_midstep=state_runtime.transfer.readable_midstep,
                slot_allocator=self.state,
            )
    ...
    def checkpoint_funnel(self) -> dict[str, int]:
        ...
        return (
            ...
            | {
                # The last stage, and the only instantaneous one: checkpoints
                # that survived every stage above and are still held, but that
                # the block index can no longer reach. `checkpoints_orphaned`
                # sees only the ones caught as their own block went, never one
                # further along the same chain.
                "checkpoints_unreachable": self.state.count_unreachable(
                    lambda h: self.kv.lookup(h) >= 0
                ),
                "slots_held": self.state.occupancy()["slots_held"],
            }
        )
