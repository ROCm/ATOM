...
class BlockManager:
    ...
    def register_received_prefix(self, seq: Sequence) -> int:
        ...
        tokens and hash-block size as the producer) lets the NEXT turn's
        `can_allocate` match this prefix locally and pull only the delta.

        Only whole hash blocks are registered; the trailing partial block is
        left unhashed (matches `hash_blocks`). Emitting KV events for the
        received blocks is left to the caller (`record_remote_store`). Returns
        the number of blocks hashed.
        """
        if not self.enable_prefix_caching:
            return 0
        hbs = self._hash_block_size()
