...
class OffloadJointRecord:
    """The KV-transfer offload protocol state for one sequence.

    These six values would otherwise be six flat attributes on ``Sequence``,
    written from ``BlockManager`` and ``Scheduler`` and read back by the offload
    connector, with the joint subset re-initialised by hand in several places --
    so a reset that forgot one field left a stale span the connector then acted
    on. Grouped here so the protocol state is one field and the joint boundary
    has exactly one place to be dropped from (``reset_joint``).
    """

    # Content hash of the checkpoint the tier was asked to fetch back into
    # `state_slot`, or -1. Equal to `boundary_hash` on a joint load; on a
    # state-only load (the KV is resident, only the recurrent state is not)
    # there is no joint boundary and this is the HBM hit's hash instead. One
    # field for "what state is being fetched", so the two cases share a path.
    state_load_hash: int = -1
    ...
    # The boundary both legs are aimed at, or 0: the rightmost checkpoint rung
    # the LMCache KV prefix covers. `num_cached_tokens` stays at the HBM prefix
    # until the load reports, which keeps the forward honest if it fails.
    ...
    # the boundary, which is at or past it. The request still claims only the
    # boundary -- see `_claim_after_load`. Overshooting costs one chunk into
    # blocks the forward is about to rewrite; claiming past it is wrong output.
    kv_tokens: int = 0
    ...
    # holds, and it is where that leg starts.
    claim_tokens: int = 0

    def reset_joint(self) -> None:
        """Drop the joint boundary and both of its transfer spans.

        The four joint fields move together -- a boundary is meaningless without
        the KV span aimed at it and the prefix claimed below it -- so every site
        that abandons a joint load clears them through here rather than by hand.
        Leaves `state_load_hash` and `kv_prefix_tokens` alone: they are set on
        their own paths and are not part of the boundary this drops.
        """
...
class Sequence:
    ...
    def __init__(
        ...
        # KV-transfer offload protocol state. Inert unless an offload connector
        # is configured; see `OffloadJointRecord`.
