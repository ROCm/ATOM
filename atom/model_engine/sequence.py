...
class Sequence:
    ...
    def __init__(
        ...
        # The last checkpoint this chain filed, for as long as the request
        # lives. `state_fork_src` cannot serve as that: the scheduler clears it
        # once a batch has carried it, so by the time `checkpoint` runs in
        # postprocess it reads -1 and no predecessor is ever found — which is
        # exactly how `supersede-events` measured 0 against 98 checkpoints
        # kept. Diagnostic only: `StateSlotPool` reads it to mark the older
        # anchor superseded, and nothing spends that mark.
