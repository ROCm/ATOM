...
def _distributed_dp_socket_plan(
    *,
    data_parallel_size: int,
    master_ip: str,
    master_port: int,
    bind_host: str = "0.0.0.0",
) -> list[DistributedDPSocketPlan]:
    """Derive deterministic coordinator/worker ZMQ endpoints.

    Ports are intentionally derived from data_parallel_master_port so users do
    not need a separate public control-port parameter.
    """
    base = master_port + 100
    return [
        DistributedDPSocketPlan(
            rank=rank,
            bind_input=f"tcp://{bind_host}:{base + rank * 2}",
            connect_input=f"tcp://{master_ip}:{base + rank * 2}",
            bind_output=f"tcp://{bind_host}:{base + rank * 2 + 1}",
            connect_output=f"tcp://{master_ip}:{base + rank * 2 + 1}",
        )
...
def _iter_dp_rank_assignments(config: Config):
    ...
    global_dp_size = pc.data_parallel_size
    distributed_dp = getattr(pc, "distributed_dp", False)
    local_dp_size = pc.data_parallel_size_local if distributed_dp else global_dp_size
    rank_offset = pc.data_parallel_rank if distributed_dp else 0

    if config.enable_dp_attention:
        for local_base in range(local_dp_size):
            for tp_rank in range(tp_size):
                global_rank = (rank_offset + local_base) * tp_size + tp_rank
                local_rank = local_base * tp_size + tp_rank
                yield global_rank, local_rank
    else:
        for local_rank in range(local_dp_size):
            yield rank_offset + local_rank, local_rank
...
class CoreManager:
    def __init__(self, config: Config):
        ...
        rank_assignments = list(_iter_dp_rank_assignments(config))
        self._distributed_dp_serving = (
            getattr(config.parallel_config, "distributed_dp", False)
            and getattr(config, "distributed_dp_serving", False)
        )
        self._distributed_coordinator = (
            not self._distributed_dp_serving
            or config.parallel_config.data_parallel_rank == 0
        )
        socket_plan = None
        if self._distributed_dp_serving:
            socket_plan = _distributed_dp_socket_plan(
        ...
        if config.enable_dp_attention:
            self.local_engine_count = len(rank_assignments)
            logger.info(
            ...
            config.parallel_config.data_parallel_size_local = self.local_engine_count
            if getattr(config.parallel_config, "distributed_dp", False):
        ...
        try:
            for dp_rank, local_dp_rank in rank_assignments:
                logger.info(
                ...
                if socket_plan is not None:
                    ...
                    addresses = {
                        "input_address": plan.connect_input,
                        "output_address": plan.connect_output,
                    }
                else:
                    addresses = None

                engine_core_process, addresses, returned_local_dp_rank = launch_engine_core(
                    rank_config, dp_rank, local_dp_rank, addresses=addresses
                )
                ...
                local_dp_ranks.append(returned_local_dp_rank)

            data_parallel = config.parallel_config.data_parallel_size > 1
            try:
                if self._distributed_coordinator and socket_plan is not None:
                    for plan in socket_plan:
                        input_socket = make_zmq_socket(
                            self.ctx, plan.bind_input, zmq.ROUTER, bind=True
                        )
                        self.input_sockets.append(input_socket)

                        output_socket = make_zmq_socket(
                            self.ctx, plan.bind_output, zmq.PULL, bind=True
                        )
                ...
                for info, local_dp_rank in zip(processes_info, local_dp_ranks):
                    ...
                    if self._distributed_coordinator and socket_plan is None:
    ...
    def close(self):
        ...
        logger.info(
            f"{self.label}: Shutting down all {len(self.input_sockets) or self.local_engine_count} EngineCores"
        )
    ...
    def add_request(self, seqs: List[Sequence]):
        ...
        engine_count = len(self.input_sockets)
        if engine_count == 1:
            ...
            for seq in seqs:
                dp_rank = self._rr_counter % engine_count
    ...
    def send_utility_command(self, cmd: str, dp_rank: int = None):
        if dp_rank is None:
            ...
            for rank in range(len(self.input_sockets)):
                logger.debug(
    ...
    def broadcast_utility_command_sync(
        ...
        responses = []
        for _ in range(len(self.input_sockets)):
            try:
    ...
    def _shutdown_engine_core_rank(self, dp_rank: int):
        try:
            ...
            if not input_socket.closed and dp_rank < len(self.engine_core_identities):
...
def launch_engine_core(
    ...
    logger.info(
        f"Creating EngineCore process: DP rank {dp_rank} (local {local_dp_rank}), will use GPUs {local_dp_rank * config.tensor_parallel_size} to {(local_dp_rank + 1) * config.tensor_parallel_size - 1}"
    )
