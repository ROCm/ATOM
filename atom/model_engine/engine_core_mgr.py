...
class CoreManager:
    ...
    def _select_dp_rank_locked(self, lo: int = 0, hi: int | None = None) -> int:
        ...
        """
        hi = self._routable_engine_count if hi is None else hi
        n = hi - lo
...
class DisaggCoreManager(CoreManager):
    ...
    def __init__(self, config: Config):
        ...
        # One set per process, not per role: under the paired topology there are
        # n_pairs prefill and n_pairs decode processes, and a shared address
        # would have them fight over the same bind.
        ...
        prefill_ctrl_addrs = [get_open_zmq_ipc_path() for _ in range(n_pairs)]
        decode_ctrl_addrs = [get_open_zmq_ipc_path() for _ in range(n_pairs)]
        for k in range(n_pairs):
            prefill_configs[k].parallel_config.control_address = prefill_ctrl_addrs[k]
            decode_configs[k].parallel_config.control_address = decode_ctrl_addrs[k]
    ...
    def add_request(self, seqs: list[Sequence]):
            ...
            if self._n_pairs == 1:
                ...
                with self._lb_lock:
                    idx = self._select_dp_rank_locked(
                        lo=self._decode_idx0,
                        hi=self._decode_idx0 + self._n_pairs,
                    )
        ...
        for pair, pair_seqs in by_pair.items():
            # Through `_send_request`, never `input_sockets` directly: it is the
            # single serialization point for the ADD socket, and two interleaved
            # send_multipart calls silently wedge the engine (see its docstring).
            decode_payload = pickle.dumps((EngineCoreRequestType.ADD, pair_seqs))
            self._send_request(self._decode_idx0 + pair, decode_payload)
            ...
            self._send_request(pair, prefill_payload)
