    ...
    def exit(self):
        ...
        for proc in self.runner_mgr.procs:
            ...
            if alive:
                ...
                # already on its way out hangs there forever. Seen twice: the
                # MainThread parks in `_exit_function -> join`, all TP workers
                # stay alive, /metrics keeps answering 200, and only a manual
                # kill recovers the node. `enable_orphan_reaping` cannot help,
                # because PR_SET_PDEATHSIG fires when the parent *dies* and this
                # parent never does.
                try:
                    if proc.is_alive():
                        logger.warning(
