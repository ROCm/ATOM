...
class ModelRunner:
    ...
    def get_num_blocks(self) -> dict[str, object]:
        ...
        #
        # Do NOT try to net out memory a peer has IPC-mapped from us. That was
        # tried: under rapidserve the decode process maps ~100GB of prefill's
        # weights while its own allocator holds ~5GB, which looks like double
        # counting. It is not — measured on 8xMI355X, the device reports those
        # pages as used and refuses to allocate past them, so subtracting them
        # hands out a KV budget that does not exist and converts a clean startup
        # error into an OOM inside allocate_kv_cache.
        ...
        try:
            ...
            gb = 1 << 30
            # The same breakdown the success path logs below. Without it here,
            # the failure path reports only the deficit, and every diagnosis of
            # "why is memory gone" becomes guesswork about which term is large.
            base_msg = (
                f"Per-request cache tensor "
                f"({exc.reserved_bytes / gb:.2f}GB for "
                f"{exc.entries} slots) exceeds available KV budget "
                f"({available_for_kv / gb:.2f}GB) at "
                f"--gpu-memory-utilization {config.gpu_memory_utilization:.2f}."
                f" Budget breakdown: total={total / gb:.2f}GB "
                f"free={free / gb:.2f}GB budget={budget / gb:.2f}GB | "
                f"peak_torch={peak_torch / gb:.2f}GB "
                f"non_torch={non_torch / gb:.2f}GB "
                f"cudagraph={cudagraph_overhead / gb:.2f}GB "
                f"safety={safety_margin / gb:.2f}GB "
                f"extra_reserve={self._kv_budget_extra_reserve(total) / gb:.2f}GB "
                f"=> non_kv_overhead={non_kv_overhead / gb:.2f}GB."
            )
...
# NOT WIRED UP -- kept as the specification for a port, not as live code.
#
# These are the plain-int dimensions KV binding depends on that the decode
# process cannot derive for itself, because it skips get_num_blocks() (it owns
# no KV memory). The branch shipped them alongside the tensors through a
# name-keyed IPC payload; #1771 replaced that with a single contiguous paged
# pool, and `num_swa_blocks` / `max_per_req_cache_slots` no longer exist as
# runner attributes at all -- their successor is `pool_plan.entries`, which the
# decode side of a P/D pair is not sent. `_bind_kv_cache_to_modules` refuses
# stateful models outright rather than binding against dims it does not have.
#
# Whatever re-enables per-request state under rapidserve has to ship the
# equivalent of this list, so the reasoning below is worth keeping:
#
...
class RapidServeModelRunner(ModelRunner):
    ...
    def __init__(self, rank, config):
        ...
        # files from different engine runs never collide. Must use the ONE
        # address every process shares — the per-decode-rank channels differ by
    ...
    def _disagg_collect_rank_files(self, tag: str) -> list[str] | None:
        ...
        if self.rank != 0:
            return None
        # Keyed by GPU index, matching what `_disagg_write_rank_file` wrote.
        # `paths[r]` is therefore the file from the worker on GPU base + r, not
        # from TP rank r -- the two coincide only in the symmetric case.
    ...
    def import_kv_cache_ipc_handle(
        ...
        # below; this side never ran sizing, so there is nothing else it could
        # come from.
        self._assert_disagg_pairing()
        path = paths[self._disagg_paths_index]
        logger.info(
    ...
    def prefill_forward(self, batch: ScheduledBatch) -> list[int]:
        ...
        With spec decode on, also runs the drafter here and returns its drafts,
        so decode's first step can speculate instead of burning a step on
        placeholder drafts. Returns ``(sampled_token_ids, draft_token_ids)``
        where draft_token_ids is one list per sequence (empty when no drafter
        ran).
        """
        ...
        with torch.cuda.stream(stream):
            ...
            # hidden_states is kept (not discarded as before): the drafter needs
            # it as target_hidden_states.
            logits, hidden_states = self.run_model(input_ids, batch)
        ...
        self._record_kv_cache_ready(batch)
        reset_forward_context()
        return sampled_cpu, drafts_cpu

    def _prefill_propose_drafts(self, batch, sampled, hidden_states) -> list:
        """Run the drafter on the prefill forward's output.

        Returns one draft list per sequence, or [] when no drafter ran — decode
        then falls back to placeholder drafts.

        Calls the drafter directly rather than going through
        propose_draft_token_ids: that wrapper ends in
        tokenIDProcessor.prepare_draft_ids, which is deferred-output plumbing —
        it stashes this step's drafts, fires an async D2H, and returns the
        PREVIOUS step's drafts (and reads `prev_req_ids`, which only
        prepare_sampled_ids inside postprocess ever sets). Prefill runs no such
        pipeline: it needs this batch's drafts synchronously, to put on the wire.

        Restricted to plain (EAGLE/MTP) drafters. DSpark is excluded on purpose:
        its ragged verification carries per-request `ell` state recorded via
        `verify_scheduler.record_ell`, and that bookkeeping is consumed in the
        DECODE process — recording it here, in a process whose scheduler never
        runs the verify step, would desynchronise it.
        """
        drafter = getattr(self, "drafter", None)
        if (
            drafter is None
            or drafter.is_block_drafter
            or drafter.uses_confidence_schedule
        ):
            return []
        bs = batch.total_seqs_num
        if bs == 0:
            return []
        # Nothing was verified this step, so nothing is rejected: the anchor is
        # each prompt's last token (prepare_inputs counts back 1+num_reject from
        # each segment end). Prefill batches skip both the RAGGED and
        # confidence-schedule branches in propose_draft_token_ids, since
        # total_tokens_num_prefill != 0.
        num_reject = self.tokenID_processor.default_num_rejected_tokens[:bs]
        # Anchor = each prompt's last token. prepare_inputs counts back
        # `1 + num_reject` from every segment end (cu_seqlens_q[1:]), and
        # nothing was verified this step, so num_reject is all-zero.
        last_token_indices = drafter.prepare_inputs(bs, 1 + num_reject)
        drafts = drafter.propose(
            target_token_ids=self.tokenID_processor.input_ids.gpu[
                1 : batch.total_tokens_num + 1
            ],
            target_positions=get_forward_context().context.positions,
            target_hidden_states=hidden_states,
            num_reject_tokens=num_reject,
            next_token_ids=sampled.view(-1),
            last_token_indices=last_token_indices,
        )
        if drafts is None:
            return []
        return drafts.view(bs, -1).tolist()
