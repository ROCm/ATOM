...
class Scheduler:
    ...
    def schedule(self) -> tuple[ScheduledBatch, dict[int, Sequence]]:
        ...
        decode_batch = ScheduledBatch(
            ...
            # An empty batch cannot execute queued maintenance, and for spills
            # that is not merely wasted work: a drained spill is only released
            # once its copy reaches a forward, so draining into a batch that is
            # never forwarded strands every slot it drained.
            state_maintenance_ops=(
