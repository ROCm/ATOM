...
def _static_routed_lora_requires_reference_path(layer: torch.nn.Module) -> bool:
    return bool(
        getattr(layer, "_static_routed_lora_requires_reference_path", False)
        or getattr(layer, "_static_routed_lora_requires_unshuffled", False)
    )


def _static_routed_lora_activation(
    gate: torch.Tensor,
    up: torch.Tensor,
    ...
    raise NotImplementedError(
        f"Static routed expert LoRA reference path does not support {activation}."
    )


def _dynamic_routed_lora_fused_context(
    layer: torch.nn.Module,
    topk_weights: torch.Tensor,
    topk_ids: torch.Tensor,
) -> dict[str, torch.Tensor] | None:
    if not getattr(layer, "_dynamic_routed_lora", False):
        return None
    wrapper = getattr(layer, "_dynamic_routed_lora_punica_wrapper", None)
    required = (
        "_dynamic_routed_lora_w13_a_stacked",
        "_dynamic_routed_lora_w13_b_stacked",
        "_dynamic_routed_lora_w2_a_stacked",
        "_dynamic_routed_lora_w2_b_stacked",
        "_dynamic_routed_lora_adapter_enabled",
        "_dynamic_routed_lora_max_loras",
    )
    if wrapper is None or not all(hasattr(layer, name) for name in required):
        return None
    return {
        "w13_a": layer._dynamic_routed_lora_w13_a_stacked,
        "w13_b": layer._dynamic_routed_lora_w13_b_stacked,
        "w2_a": layer._dynamic_routed_lora_w2_a_stacked,
        "w2_b": layer._dynamic_routed_lora_w2_b_stacked,
        "topk_weights": topk_weights,
        "topk_ids": topk_ids,
        "punica_wrapper": wrapper,
        "adapter_enabled": layer._dynamic_routed_lora_adapter_enabled,
        "max_loras": int(layer._dynamic_routed_lora_max_loras),
    }


def _module_has_dynamic_routed_lora(layer: torch.nn.Module) -> bool:
    return bool(getattr(layer, "_dynamic_routed_lora", False))
...
def _apply_static_routed_lora_vllm_triton_fp8(
    ...
    *,
    ...
    intermediate_cache1 = torch.empty((M, topk, N), dtype=x.dtype, device=x.device)
    activation_out_dim = N // 2
    intermediate_cache2 = torch.empty(
        (M * topk, activation_out_dim), dtype=x.dtype, device=x.device
    )
    intermediate_cache3 = torch.empty((M, topk, K), dtype=x.dtype, device=x.device)
...
def _apply_dynamic_routed_lora_vllm_triton_fp8(
    layer: torch.nn.Module,
    x: torch.Tensor,
    topk_weights: torch.Tensor,
    topk_ids: torch.Tensor,
    ...
    *,
    ...
    if activation == ActivationType.Silu:
        ...
        raise NotImplementedError(
            f"Dynamic routed LoRA FP8 Triton path does not support {activation}."
        )
    ...
    dynamic_lora = _dynamic_routed_lora_fused_context(layer, topk_weights, topk_ids)
    assert dynamic_lora is not None
    punica_wrapper = dynamic_lora["punica_wrapper"]
    w13_lora_a = dynamic_lora["w13_a"]
    w13_lora_b = dynamic_lora["w13_b"]
    w2_lora_a = dynamic_lora["w2_a"]
    w2_lora_b = dynamic_lora["w2_b"]
    adapter_enabled = dynamic_lora["adapter_enabled"]
    max_loras = int(dynamic_lora["max_loras"])
    ...
    sorted_token_ids_lora, expert_ids_lora, num_tokens_post_padded_lora = (
        punica_wrapper.moe_lora_align_block_size(
            topk_ids.to(torch.int32),
            M,
            w13_shrink_config["BLOCK_SIZE_M"],
            global_num_experts,
            max_loras,
            adapter_enabled,
            expert_map,
    ...
    token_lora_mapping, _, _, _, lora_ids, _ = (
        punica_wrapper.token_mapping_meta.meta_args(M)
    )
...
def _store_static_routed_lora_stack(
    ...
    if stack_name == "w13":
        ...
        layer._static_routed_lora_w2_a_stacked[stack_idx, local_expert_id].copy_(lora_a)
        layer._static_routed_lora_w2_b_stacked[stack_idx, local_expert_id].copy_(lora_b)
...
def _apply_static_routed_lora_reference(
    ...
    for global_expert_id in selected_expert_ids:
        ...
        if local_expert_map is not None:
            if global_expert_id >= int(local_expert_map.numel()):
                continue
            mapped = int(local_expert_map[global_expert_id].item())
            if mapped == -1:
                continue
        ...
        if w13_bias is not None:
            gate = gate + w13_bias[local_expert_id, :shard_size].to(gate.dtype)
            up = up + w13_bias[local_expert_id, shard_size : shard_size * 2].to(
                up.dtype
            )
        ...
        hidden = _static_routed_lora_activation(gate, up, activation)
        ...
        if w2_bias is not None:
            down_out = down_out + w2_bias[local_expert_id].to(down_out.dtype)
    ...
    def apply(
        ...
        if self.use_triton:
            ...
            if needs_custom_routing:
                ...
                if _static_routed_lora_requires_reference_path(layer):
                    return _apply_static_routed_lora_reference(
                        layer,
                        x,
                        topk_weights,
...
class CompressedTensorsFp8MoEMethod(FusedMoEMethodBase):
    ...
    def apply(
        ...
        if _module_has_dynamic_routed_lora(layer):
            block_shape = [self.block_n, self.block_k] if self.block_quant else None
            return _apply_dynamic_routed_lora_vllm_triton_fp8(
                layer,
                x,
                topk_weights,
                topk_ids,
...
class FusedMoE(torch.nn.Module):
    ...
    def add_static_routed_lora(
        ...
        if local_expert_id == -1:
            return
        if self.activation != ActivationType.Silu or self.has_bias:
            self._static_routed_lora_requires_reference_path = True
        if proj in self._static_routed_lora.get(expert_id, {}):
            raise ValueError(
                "Static routed expert LoRA fused stack supports one adapter per "
                f"expert/projection; duplicate target expert={expert_id} proj={proj}"
            )
