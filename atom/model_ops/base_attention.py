...
def unified_attention_into_output(
    ...
    reads a stale pointer. That is caught by CUDAGraphWrapper's input_addresses
    assert only while ``is_debugging_mode`` is on; with it off the replay reads
    the stale memory and the model quietly emits garbage. Same contract as
    vLLM's ``*_with_output`` ops and ``kimi_k3.kda_core_with_output``.

    Added alongside ``unified_attention_with_output_base`` rather than replacing
    it: that op is also called from the SGLang plugin, which runs its own
    runtime and does not need this. Its dispatch is repeated below instead of
    being factored out, to keep this change from touching the op the plugin
    path depends on -- so keep the two in sync.
