...
def _dual_stream_is_active(self, num_tokens: int) -> bool:
    ...
    # mask, so a dual-stream fork escapes the partition and contends with the
    # other process. The gate below was added for that and then deliberately
    # switched off by the "overlap enabled" commit, which is measuring whether
    # the contention is worth the overlap. Kept here, disabled, because the
