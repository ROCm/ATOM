...
class GatedDeltaNet(nn.Module):
    ...
    def forward(
        ...
        if gdn_metadata.num_prefills > 0:
            ...
            #
            # ReplaySSM never populates `ssm_checkpoints` (it reconstructs
            # interiors on demand from the record ring instead of snapshotting
            # them), so `ckpt is None` there and this copy-out is skipped.
            if ckpt is not None:
