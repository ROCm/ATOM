...
class Sampler(nn.Module):
    ...
    def _temperature_sample(
        ...
        ATOM_USE_TORCH_SAMPLER (ATOM_GFX1250_WORKAROUND): aiter HIP
        mixed_sample_outer_exponential faults on gfx1250 (opus::gmem
        buffer-resource issue). When the flag is set, fall back to torch-native
        greedy argmax. Simplification: this path is greedy only (bring-up uses
        temperature=0) and does NOT reproduce the non-greedy / n>1 noise
        behaviour. Default off => original Gumbel-max.
        """
