...
One topic across four modules -- how many entries a byte budget buys
(`sub_pool_spec`), which row of the unified pool a DeepSeek-V4 layer's window
or compressed group occupies (`v4_pool_geometry`), one request's whole state as
a contiguous run (`state_arena`), and how that run is scattered across PAGE
units and gathered back (`paged_state_copy`). All of it answers a question
...
Nothing is re-exported here: a convenience import would pull all four in
