...
class GDNAttentionMetadataBuilder(AiterAttentionMetadataBuilder):
    ...
    def prepare_state_indices(self, batch: ScheduledBatch, with_spec: bool = False):
        ...
        for idx, mamba_block_table in enumerate(batch.mamba_block_tables):
            ...
            if not with_spec:
                non_spec_state_indices[idx, 0] = mamba_block_table[0]
            else:
    ...
    def prepare_decode(  # type: ignore[override]
        ...
        if self.use_spec_decode:
            ...
            self.spec_query_start_loc[: num_decodes + 1].copy_(
                gdn_metadata.spec_query_start_loc, non_blocking=True
            )
            ...
            self.num_accepted_tokens[:num_decodes].copy_(
                gdn_metadata.num_accepted_tokens, non_blocking=True
            )
            ...
            self.non_spec_query_start_loc[: num_decodes + 1].copy_(
                gdn_metadata.non_spec_query_start_loc, non_blocking=True)
    ...
    def build_for_cudagraph_capture(
        ...
        self.num_accepted_tokens[:bs].copy_(num_accepted_tokens, non_blocking=True)
        if self.use_spec_decode:
            gdn_metadata = GDNAttentionMetadata(
                ...
                num_decode_tokens=0,
                num_spec_decodes=bs,
                num_spec_decode_tokens=bs * self.num_spec,
                num_actual_tokens=bs * self.num_spec,
                has_initial_state=None,
                spec_query_start_loc=self.spec_query_start_loc[: bs + 1],
                non_spec_query_start_loc=None,
                spec_state_indices_tensor=self.spec_state_indices_tensor[:bs],
                non_spec_state_indices_tensor=None,
            ...
            gdn_metadata = GDNAttentionMetadata(
                ...
                spec_state_indices_tensor=None,
                non_spec_state_indices_tensor=self.non_spec_state_indices_tensor[:bs],
                spec_sequence_masks=None,
                spec_token_indx=None,
                non_spec_token_indx=None,
                num_accepted_tokens=None,
