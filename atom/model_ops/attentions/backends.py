...
class AttentionMetadataBuilder(ABC, Generic[T]):
    ...
    #: Whether this backend's per-request state lives in the SAME allocation
    #: the paged blocks come from, rather than in a tensor of its own.
    #:
    #: It settles one thing the allocator cannot see for itself: how many slot
    #: indices may ever exist. A private tensor is sized once at startup and an
    #: index past its slot dimension is an out-of-bounds GPU read, so the count
    #: it was given is a hard ceiling. A unified pool gives every superblock a
    #: slot, so the superblock supply is the ceiling — the same count that
    #: already governs the bytes, rather than a second, tighter one.
    uses_unified_pool: bool = False
