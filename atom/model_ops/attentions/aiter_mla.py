...
class AiterMLAMetadataBuilder(CommonAttentionBuilder):
    ...
    def prepare_mixed(self, batch: ScheduledBatch, bs: int):
        ...
        The prefill half reuses the tested `prepare_prefill`, run against a
        PRIVATE mirror of `forward_vars` (see `_get_mixed_prefill_bank`) so its
        staged metadata neither shares a GPU destination nor a pinned CPU source
        with the decode half below. That isolation is what lets us skip the
