...
def is_rocm_aiter_fusion_shared_expert_enabled_for_quant_config(
    ...
    if dp_size > 1 and config.moe_ep_flatten_tp_across_dp:
        return False
    # MoRI resolves the destination rank from the raw expert id, so the AITER
    # tail id has no owner there; the MoE layer places it instead. EPLB always
    # does, otherwise the env decides.
