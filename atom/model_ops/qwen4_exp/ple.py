...
class _Qwen4ExpFP8Embedding(nn.Module):
    ...
    def forward(self, ids: torch.Tensor) -> torch.Tensor:
        ...
        output = torch.empty(
            (ids.numel(), dim), dtype=self.output_dtype, device=ids.device
        )
        if ids.numel() == 0:
            return output
...
class Qwen4ExpNGramEmbedding(nn.Module):
    ...
    def compute_ngram_ids(
        ...
        """Hash flat tokens without a [requests, max_length, heads] workspace.

        Only ngram_size - 1 preceding tokens are consulted. A previous EOS
        resets all longer histories, including histories crossing chunks.
        """
        input_ids = input_ids.reshape(-1)
        num_tokens = input_ids.numel()
        num_reqs = query_start_loc.numel() - 1
        if num_tokens > self.max_total_tokens or num_reqs > self.max_num_reqs:
        ...
        if ngram_context.shape != (num_reqs, self.ngram_size - 1):
            raise ValueError("PLE ngram_context must be [requests, ngram_size - 1]")
        out = torch.empty(
            (num_tokens, self.ngram_heads), dtype=torch.int64, device=input_ids.device
        )
        if not num_tokens:
            return out
        if num_reqs == 0:
        ...
        _ngram_ids_kernel[(triton.cdiv(num_tokens, 32),)](
            ...
            BLOCK_TOKENS=32,
            BLOCK_HEADS=triton.next_power_of_2(self.heads_per_ngram),
            num_warps=4,
        )
        return out
