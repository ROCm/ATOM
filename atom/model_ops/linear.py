...
class LinearBase(nn.Module):
    ...
    def _gather_full_weight_via(self, weight, group):
        ...
        if self.tp_size <= 1 or self.tp_dim is None:
            return weight
        # RCCL cannot reliably all_gather FP8/E8M0 dtypes. Gather raw bytes and
        # reinterpret afterwards; this applies to projection weights as well as
        # quantization scales and is bit-exact.
        fp8_dtypes = {
            dtype
            for dtype in (
        ...
        if weight.dtype in fp8_dtypes:
            ...
            return gathered.view(original_dtype)
