...
class MinimaxM3IndexerParallelLinear(ColumnParallelLinear):
    """MiniMax-M3 lightning-indexer projection for ``[index_q | index_k]``.

    Splitting this from the main QKV projection lets sparse layers that reuse a
    cached top-k skip the indexer GEMM entirely.
    """
    ...
    def _shard_offset_size(self, loaded_shard_id: str) -> tuple[int, int]:
        index_q_size = self.num_index_heads * self.index_head_size
        mapping = {
            "index_q": (0, index_q_size),
            "index_k": (index_q_size, self.index_head_size),
        }
        if loaded_shard_id not in mapping:
            raise ValueError(
                "MiniMax-M3 indexer shard id must be one of "
                "'index_q', 'index_k'; got "
                f"{loaded_shard_id!r}."
            )
        return mapping[loaded_shard_id]
    ...
    def weight_loader(
        ...
        if param is getattr(self, "weight_scale", None) or param is getattr(
            ...
            if self.quant_type == QuantType.per_1x128:
                ...
                loaded_weight = loaded_weight.view(1, 1).repeat(self.tp_size, 1)
                shard_offset = ["index_q", "index_k"].index(loaded_shard_id)
                shard_size = 1
