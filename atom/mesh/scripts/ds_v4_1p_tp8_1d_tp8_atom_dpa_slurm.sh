#!/usr/bin/env bash
#SBATCH --job-name=ds-v4-1p-tp8-1d-tp8-atom-dpa
...
#SBATCH --output=/it-share/yajizhan/slurm_logs/ds_v4_1p_tp8_1d_tp8_atom_dpa-%j.out
#SBATCH --error=/it-share/yajizhan/slurm_logs/ds_v4_1p_tp8_1d_tp8_atom_dpa-%j.err
#
# Self-contained 1P+1D PD-disaggregated benchmark for DeepSeek-V4-Pro
# on ATOM with mooncake RDMA KV transfer.
#   prefill: TP=8, decode: TP=8, --enable-dp-attention.
#
# Usage:
#   mkdir -p /it-share/yajizhan/slurm_logs
#   sbatch ds_v4_1p_tp8_1d_tp8_atom_dpa_slurm.sh
...
CONTAINER="${CONTAINER:-atom_mesh_dsv4_1p1d_dpa_${SLURM_JOB_ID}}"
...
OSL="${OSL:-1024}"
CONC_LIST="${CONC_LIST:-32,64,128,256,512}"
...
LOG_ROOT="${LOG_ROOT:-/it-share/yajizhan/slurm_logs/$(date +%m%d)_ds_v4_1p_tp8_1d_tp8_atom_dpa_${SLURM_JOB_ID}}"
...
RUN_TAG="$(date +%Y%m%d%H%M%S)_gsm8k_v4_1p1d_dpa"
mkdir -p "${RESULT_DIR}"
...
for ISL in "${ISLS[@]}"; do
    for CONC in "${CONCS[@]}"; do
        RESULT_FILENAME="pd-atom-v4-1p1d-dpa-${ISL}-${OSL}-${CONC}-${RANDOM_RANGE_RATIO}"
        echo ""
        echo "========================================="
...
json_files = sorted(result_dir.glob('pd-atom-v4-1p1d-dpa-*.json'))
if not json_files:
