#!/usr/bin/env bash
#SBATCH --job-name=ds-fp4-atom-1p-tp8-1d-tp8
...
#   sbatch ds_fp4_1p_tp8_1d_tp8_atom_slurm.sh
...
# Local ATOM source tree (PYTHONPATH override so /kv_transfer_info endpoint
# from this branch is picked up instead of /app/ATOM in the image).
...
LOG_ROOT="${LOG_ROOT:-/it-share/yajizhan/slurm_logs/$(date +%m%d)_ds_fp4_atom_1p_tp8_1d_tp8_${SLURM_JOB_ID}}"
...
# Mesh router from /it-share working tree (commit 7d53515e adds --backend atom).
# Image's /usr/local/bin/atom-mesh is pre-built from main and does NOT have the
# new variant.
echo ""
...
"
# Copy the same binary into the decode container (cargo target dir lives on
# /it-share so it's already visible; just install it).
srun --nodelist="$DECODE_NODE" --nodes=1 --ntasks=1 bash -lc "
    docker exec '${CONTAINER}' cp '${MESH_SRC}/target/release/atom-mesh' /usr/local/bin/atom-mesh
" || true   # decode node doesn't actually run the router but keep it consistent
