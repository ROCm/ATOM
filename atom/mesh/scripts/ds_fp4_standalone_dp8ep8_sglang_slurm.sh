#!/usr/bin/env bash
#SBATCH --job-name=ds-fp4-standalone-dp8ep8-sglang
...
#SBATCH --output=/it-share/yajizhan/slurm_logs/ds_fp4_standalone_dp8ep8_sglang-%j.out
#SBATCH --error=/it-share/yajizhan/slurm_logs/ds_fp4_standalone_dp8ep8_sglang-%j.err
#
# Self-contained single-node (non-disaggregated) benchmark for DeepSeek-R1 MXFP4
# on SGLang with DP=8, EP=8, --enable-dp-attention.
# Used to isolate DP+EP accuracy vs PD-disaggregation accuracy regressions.
#
# Usage:
#   mkdir -p /it-share/yajizhan/slurm_logs
#   sbatch ds_fp4_standalone_dp8ep8_sglang_slurm.sh
...
CONTAINER="${CONTAINER:-atom_sglang_standalone_dp8ep8_${SLURM_JOB_ID}}"
...
LOG_ROOT="${LOG_ROOT:-/it-share/yajizhan/slurm_logs/$(date +%m%d)_ds_fp4_standalone_dp8ep8_sglang_${SLURM_JOB_ID}}"
...
MODEL    : ${MODEL_PATH}
IMAGE    : ${DOCKER_IMAGE}
BACKEND  : sglang (standalone DP+EP, no PD)
...
RUN_TAG="$(date +%Y%m%d%H%M%S)_gsm8k_dp8ep8"
mkdir -p "${RESULT_DIR}"
...
for PAIR in "${PAIRS[@]}"; do
    ...
    for CONC in "${CONCS[@]}"; do
        RESULT_FILENAME="standalone-dp8ep8-sglang-${ISL}-${OSL}-${CONC}-${EFFECTIVE_RATIO}"
        echo ""
        echo "========================================="
...
json_files = sorted(result_dir.glob('standalone-dp8ep8-sglang-*.json'))
if not json_files:
...
echo "[server] launching standalone sglang (DP=${DP}, EP=${EP}) on ${NODE}"
srun --nodelist="$NODE" --nodes=1 --ntasks=1 bash -lc "
