#!/usr/bin/env bash
#SBATCH --job-name=ds-fp4-standalone-tp8-atom
...
#SBATCH --output=/it-share/yajizhan/slurm_logs/ds_fp4_standalone_tp8_atom-%j.out
#SBATCH --error=/it-share/yajizhan/slurm_logs/ds_fp4_standalone_tp8_atom-%j.err
#
# Self-contained single-node (non-disaggregated) benchmark for DeepSeek-R1 MXFP4
# on ATOM native server. Mirrors ds_fp4_standalone_tp8_sglang_slurm.sh structure
# but uses ATOM entrypoint — used to isolate model vs PD-framework regressions.
#
# Workload sweep: ISL:OSL pairs (default "8192:1,1:1024,8192:1024") x CONC list.
#
# Usage:
#   mkdir -p /it-share/yajizhan/slurm_logs
#   sbatch ds_fp4_standalone_tp8_atom_slurm.sh
#
# Override defaults via env, e.g.:
#   sbatch --export=ALL,ISL_OSL_LIST="8192:1024" ds_fp4_standalone_tp8_atom_slurm.sh
...
LOG_ROOT="${LOG_ROOT:-/it-share/yajizhan/slurm_logs/$(date +%m%d)_ds_fp4_standalone_tp8_atom_${SLURM_JOB_ID}}"
...
for PAIR in "${PAIRS[@]}"; do
    ...
    for CONC in "${CONCS[@]}"; do
        RESULT_FILENAME="standalone-atom-${ISL}-${OSL}-${CONC}-${EFFECTIVE_RATIO}"
        echo ""
        echo "========================================="
...
json_files = sorted(result_dir.glob('standalone-atom-*.json'))
if not json_files:
...
echo "[server] launching standalone ATOM on ${NODE}"
srun --nodelist="$NODE" --nodes=1 --ntasks=1 bash -lc "
