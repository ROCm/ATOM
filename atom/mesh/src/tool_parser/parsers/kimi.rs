...
mod k3 {
    //! Kimi-K3 channel-format tool-call parser.
    ...
    impl KimiK3Parser {
        pub fn new() -> Self {
            Self {
                call: Regex::new(
                    r#"(?s)<\|open\|>call tool="([^"]*)"(?:\s+index="\d+")?<\|sep\|>(.*?)<\|close\|>call"#,
                )
                .expect("valid Kimi K3 call pattern"),
                argument: Regex::new(
                    r#"(?s)<\|open\|>argument key="([^"]*)"(?:\s+type="([^"]*)")?<\|sep\|>(.*?)<\|close\|>argument"#,
                )
                .expect("valid Kimi K3 argument pattern"),
                framing: Regex::new(
                    r#"(?s)<\|(?:open|close)\|>(?:response|message|tools|think|call|argument)[^<]*?<\|sep\|>|<\|close\|>(?:response|message|tools|think)|<\|end_of_msg\|>|<\|sep\|>"#,
                )
                .expect("valid Kimi K3 framing pattern"),
                buffer: String::new(),
                emitted: false,
            }
        }

        pub fn is_kimi_k3(text: &str) -> bool {
            [
                r#"<|open|>call tool=""#,
                "<|open|>tools<|sep|>",
                "<|open|>response<|sep|>",
        ...
        fn parse_calls(&self, text: &str) -> ParserResult<(String, Vec<ToolCall>)> {
            let tools_start = text.find("<|open|>tools<|sep|>");
            let content_source = tools_start.map_or(text, |index| &text[..index]);
            let content = self
                .framing
                .replace_all(content_source, "")
                .trim()
                .to_string();
            let mut calls = Vec::new();
            for call in self.call.captures_iter(text) {
                let name = call.get(1).expect("call name").as_str().to_string();
                let body = call.get(2).expect("call body").as_str();
                let mut args = Map::new();
                for argument in self.argument.captures_iter(body) {
                    args.insert(
                        argument.get(1).expect("argument key").as_str().to_string(),
                        Self::coerce(
                            argument.get(3).expect("argument value").as_str(),
                            argument.get(2).map(|capture| capture.as_str()),
                        ),
                    );
                }
                calls.push(ToolCall {
    ...
    impl ToolParser for KimiK3Parser {
        ...
        async fn parse_incremental(
            ...
            self.buffer.push_str(chunk);
            if !self.buffer.contains("<|end_of_msg|>") {
                return Ok(StreamingParseResult::default());
            }
            ...
            Ok(StreamingParseResult {
                ...
                calls: calls
                    .into_iter()
                    .enumerate()
                    .map(|(tool_index, call)| ToolCallItem {
                        tool_index,
    ...
    mod tests {
        ...
        async fn parses_kimi_k3_typed_arguments() {
            ...
            let (content, calls) = parser
                .parse_complete(
                    "<|open|>response<|sep|>hello<|close|>response<|sep|><|open|>tools<|sep|><|open|>call tool=\"weather\" index=\"0\"<|sep|><|open|>argument key=\"days\" type=\"int\"<|sep|>3<|close|>argument<|sep|><|close|>call<|end_of_msg|>",
                )
                .await
                .unwrap();
