...
# A declared string is read to its own closing tag rather than to the next tag
# that looks like a parameter. `_PARAM_RE` stops at any `<word>`, which is right
# for a value that cannot contain one and wrong for a value that can: HTML, code
# or XML in a `string` argument came back cut at its first inner tag, so a tool
# writing a file wrote a truncated one.
_STRING_TYPES = frozenset({"string", "str", "text", "varchar", "char", "enum"})


def _string_span(body: str, name: str) -> tuple[int, int, str] | None:
    """``(start, end, value)`` of ``<name>...</name>``, or None if absent."""
    match = re.search(
        r"<" + re.escape(name) + r">(.*?)" + _NS + r"</" + re.escape(name) + r">",
        body,
        re.DOTALL,
    )
    if match is None:
...
# The tokenizer marks 35 of MiniMax-M3's added tokens as special, and several
# are shaped like a parameter's opening tag (`<filename>`, `<filepath>`,
# `<file_content>`). `decode(skip_special_tokens=True)` erases those, leaving a
# closing tag with no opener, and the argument disappeared with no error --
# required ones included.
# Matched on a copy with the ns_token removed: the token contains a `>`, so on
# the raw bytes the anchor lands inside it and the value picks up its tail.
_ORPHAN_CLOSE_RE = re.compile(r"(?:\A|>)([^<>]*?)</([\w-]+)>")


def _orphaned_values(body: str) -> list[tuple[str, str]]:
    """``(name, value)`` for each closing tag whose opening tag never arrived."""
    clean = body.replace(MINIMAX_NS, "")
    found: list[tuple[str, str]] = []
    for match in _ORPHAN_CLOSE_RE.finditer(clean):
        value, name = match.group(1), match.group(2)
        if f"<{name}>" in clean[: match.start()]:
            continue
...
class MiniMaxParser(ToolCallParser):
    ...
    def parse_region(
        ...
        for m in _INVOKE_RE.finditer(region):
            ...
            args: dict[str, Any] = {}
            # Declared strings come out first and are then blanked from the body:
            # their markup would otherwise be read as more parameters, so
            # `save_snippet(code="<h1>Hi</h1>")` also produced an `h1` argument.
            rest = body
            for k, declared in types.items():
                if str(declared).lower() not in _STRING_TYPES:
                    continue
                span = _string_span(rest, k)
                if span is None:
                    continue
                start, end, value = span
                args[k] = coerce_json_or_raw(value, declared, strip_framing=False)
                rest = rest[:start] + " " * (end - start) + rest[end:]
            for pm in _PARAM_RE.finditer(rest):
                k = pm.group(1).strip()
                if k and k not in args:
                    args[k] = coerce_json_or_raw(
                        pm.group(2), types.get(k), strip_framing=False
                    )
            for k, value in _orphaned_values(rest):
                # Only a declared parameter: a stray `</b>` in prose is not an
                # argument whose opening tag went missing.
                if k in types and k not in args and value.strip():
                    args[k] = coerce_json_or_raw(
                        value, types.get(k), strip_framing=False
                    )
            spans.append(
