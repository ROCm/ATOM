#!/bin/bash
# GLM-5.2 PP4×TP1 Prefill + TP4 Decode (PD) with SparseKV ENABLED on decode.
#
# Same topology as start_glm52_pp4pd.sh, but the DECODE node runs:
#   - --level 3 CUDAGraph: the fused GPU hot path (ATOM_SPARSEKV_FUSED=1) is
...
#   - ATOM_SPARSEKV_ENABLE=1 (decode node only).
#
# Prefill is unchanged (SparseKV is a decode-side overlay). The full GPU KV cache
...
# ── SparseKV decode-node knobs (keep small to bound the CPU cold pool) ──
...
# ── decode: TP4 on GPU 4-7, CUDAGraph (level 3) + SparseKV ──────────
echo ">>> Starting decode (TP4, level 3 CUDAGraph, SparseKV) on GPU 4-7 ..."
AITER_LOG_LEVEL=WARNING \
AITER_QUICK_REDUCE_QUANTIZATION=INT4 \
AITER_USE_FLYDSL_MOE_SORTING=1 \
ATOM_SPARSEKV_ENABLE=1 \
ATOM_SPARSEKV_HOT_BUFFER_SIZE=$HS_HOT_BUFFER_SIZE \
ATOM_SPARSEKV_RDMA_DIRECT=$HS_RDMA_DIRECT \
...
echo ""
echo "=== All services up (SparseKV ENABLED on decode) ==="
echo "  prefill log: /tmp/prefill.log"
echo "  decode  log: /tmp/decode.log   (grep SparseKVCoordinator to confirm alloc)"
