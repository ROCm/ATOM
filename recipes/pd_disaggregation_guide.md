...
## Reusing the prefill's token ids

Both halves of a PD pair are given the same request body, so by default both
render the chat template from `messages` and tokenize the result — the decode
node repeating, on its own critical path, work the prefill node has already
done. On an agentic workload (68 k-token prompts, GLM-5.2, 1P1D) that
duplicate was **203 ms, 17.6% of decode TTFT**; see
[docs/ttft_breakdown_guide.md](../docs/ttft_breakdown_guide.md).

atomesh removes it by carrying the ids across, following vLLM's
disaggregated-prefill protocol so the two are wire-compatible:

1. Router → prefill: `return_token_ids: true` (alongside the `stream: false` /
   `max_tokens: 1` it already injects).
2. Prefill → router: the ids on the response's top-level `prompt_token_ids`.
3. Router → decode: those ids in `kv_transfer_params["prompt_token_ids"]`,
   injected next to the `remote_*` fields it already enriches.
4. Decode: skips both the template render and the tokenize. `messages` is still
   required and still validated, but its content is not read.

**This needs no configuration and has no flag.** It also degrades on its own: a
prefill node that does not answer `return_token_ids` leaves nothing to carry,
and decode tokenizes exactly as before — slower, still correct. That makes the
prefill and decode nodes independently upgradable.

Because the ids come from a single tokenizer call on the prefill node rather
than from two, they cannot drift between P and D. This is why the work is done
there and not in the router: rendering ATOM's prompts requires Python. Models
like DeepSeek-V4 and Kimi-K3 ship a `<model>/encoding/encoding_*.py` encoder
that `apply_chat_template` imports and executes instead of a Jinja template
(see `atom/entrypoints/openai/chat_encoders.py`), which the Rust router cannot
reproduce. SGLang's router-side tokenization works because it applies only to
its gRPC mode, where a Rust tokenizer owns the whole path.

### Using it directly

The same fields work without a router, so a pre-tokenized prompt is also a
first-class input for benchmarking and for clients that tokenize themselves:

```bash
# Ask a node for the ids it computed.
curl localhost:8010/v1/chat/completions -H 'Content-Type: application/json' -d '{
  "model": "m", "messages": [{"role": "user", "content": "hi"}],
  "max_tokens": 1, "return_token_ids": true
}'   # -> {..., "prompt_token_ids": [...]}

# Send a prompt that is already tokenized.
curl localhost:8020/v1/completions -H 'Content-Type: application/json' -d '{
  "model": "m", "prompt_token_ids": [100, 101, 102], "max_tokens": 16
}'
```

On `/v1/chat/completions` and `/v1/completions`, `prompt_token_ids` is accepted
both at the top level and inside `kv_transfer_params` (the vLLM location); the
two must agree or the request is rejected. `return_token_ids` is answered on
any non-streaming request, including `n > 1` — the siblings share one prompt,
tokenized once, so there is one list to report. It is rejected on a stream
rather than ignored, because a stream has no response body to put the ids on
and answering with nothing would put the tokenize cost back with nothing to
attribute it to.
