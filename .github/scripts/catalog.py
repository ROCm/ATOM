...
def grid_prefixes(path: str | Path) -> set[str]:
    """Prefixes that belong in the random model-checkbox grid.

    A prefix earns a grid checkbox by having at least one non-agentic variant.
    Agentic-only prefixes (every variant is ``aiperf_agentic``) are selected by
    the dedicated agentic dispatch inputs instead, so they stay out of the grid.
    """
    return {
        v["prefix"] for v in load_variants(path) if v["bench_kind"] != "aiperf_agentic"
    }


def validate_dispatch_inputs(path: str | Path, input_keys: set[str]) -> list[str]:
    """Check the workflow_dispatch model-grid toggles stay in sync with the catalog.

    Returns a list of human-readable problems (empty == in sync). Only the
    random model-grid toggles are expected to match prefixes -- callers pass that
    subset of input keys -- and they mirror the grid prefixes (those with a
    non-agentic variant), not every catalog prefix.
    """
    prefixes = grid_prefixes(path)
    problems: list[str] = []
