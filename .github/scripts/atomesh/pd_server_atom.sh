...
# GPU timing is opt-in; agentic reports need it on all service roles.
if [[ "${BENCHMARK_KIND}" == "aiperf_agentic" ]]; then
  ...
  export ATOMESH_STANDALONE_ENV_ATOM_ENABLE_METRICS_DEVICE_TIMER="${ATOMESH_STANDALONE_ENV_ATOM_ENABLE_METRICS_DEVICE_TIMER:-${ATOM_ENABLE_METRICS_DEVICE_TIMER:-1}}"
fi
