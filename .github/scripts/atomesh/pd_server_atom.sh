...
# agentx-v1.0.4 (see .github/benchmark/models_atomesh.yaml for why it is a SHA)
...
# ${ROLE_IP} and ${HANDSHAKE_PORT} are the two placeholders a case's `env` block
# may use; both are only known here, after the node IP is resolved and after
# ATOMESH_SERVICE_PORT_OFFSET has been folded into the port. Cases that override
# PREFILL_KV_TRANSFER_CONFIG must spell the port as ${HANDSHAKE_PORT} rather than
# hardcoding 6301 — the eval phase restarts every server with
# ATOMESH_RESTART_PORT_OFFSET applied (pd_slurm_job.sh:28-34), so a literal port
# would leave the producer advertising the benchmark-phase port while the
# consumer connects to the shifted one.
apply_prefixed_env() {
  local prefix="$1"
  local role_ip="$2"
